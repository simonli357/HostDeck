...
`smoke:dashboard-android` is the strict `FE-V1-090` no-retry aggregate production-dashboard run over cellular plus private Tailscale HTTPS. It requires the same clean committed worktree, exact saved-profile ids, unlocked authorized phone, Chrome, Tailscale, sufficient battery, and TalkBack. The harness uses one automated no-QR pairing link and the production app/routes to traverse all 39 mobile interactions and 54 physical state checkpoints, including Mission Control, Session Detail, prompt, `/model`, `/goal`, `/plan`, utilities, approval, diagnostics, interrupt, archive, resume copy, device revoke, lock/local unlock, profile recovery, runtime compatibility, TalkBack, and self-revoke. It restores the acceptance environment and publishes privacy-checked screenshots plus JSON under `artifacts/fe-v1-090-mobile-dashboard-physical-hardening/physical-android/`; final retained deployment is a separate step.
