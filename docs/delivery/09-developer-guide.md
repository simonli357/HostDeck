...
| Production executable smoke | `HOSTDECK_CODEX_BIN=/absolute/path/to/codex-0.144.0 pnpm smoke:executable-serve` | Runs the direct read-only packaged command twice with test-owned assets, exact no-model Codex, protected loopback HTTP/static checks, signal shutdown, same-port reuse, and residue inspection. The command reference includes the validation-only unsupported-version diagnostic mode. |
...
The ordinary package still lacks the real Vite `web/` tree, persistent user-unit installation/lifecycle, and clean-machine parity. It contains the verified pure two-unit generator, but does not write an environment file, install, enable, or manage units. Do not publish foreground or service-wrapper user instructions until `IFC-V1-053` and `IFC-V1-056` to `IFC-V1-058` close those paths.
