...
| Production package build | `pnpm build` | Offline frozen-lock build of `dist/hostdeck` from the exact 619-source server/CLI closure plus a fresh pinned Vite 8.1.4 dashboard build. Emits six compiled packages, production dependencies, real `web/` assets and strict web manifest, one `codexdeck` executable, one non-executable `dist/service-host.js`, the systemd lifecycle owners, schema-4 package identity, and the dependency-free verifier. |
...
| Production executable smoke | `HOSTDECK_CODEX_BIN=/absolute/path/to/codex-0.144.0 pnpm smoke:executable-serve` | Runs the direct read-only packaged command twice with the real manifest-verified dashboard, exact no-model Codex, protected loopback HTTP/static checks, signal shutdown, same-port reuse, and residue inspection. The command reference includes the validation-only unsupported-version diagnostic mode. |
...
| Production persistent lifecycle smoke | `HOSTDECK_CODEX_BIN=/absolute/path/to/codex-0.144.0 pnpm smoke:service-lifecycle` | On supported Linux/systemd, builds isolated verified versions and exercises actual persistent install/idempotence/start/status/restart/stop, inactive and active upgrades, injected rollback, Codex continuity, Tailscale noninterference, state/config preservation, and exact owned cleanup. |
...
`IFC-V1-054` connects the one compiled command to the accepted 22-registration/35-route application and foreground listener. It derives assets from the package root, resolves canonical Codex from `HOSTDECK_CODEX_BIN` or bounded absolute `PATH` entries, publishes one loopback readiness line, waits for terminal truth, and has no old custom-listener, tmux, direct-LAN TLS, cwd, or historical-route fallback. `IFC-V1-087` probes that binary before runtime admission: exact 0.144.0 continues normally, a valid different semver skips app-server start/attachment and publishes only the mutation-closed diagnostic listener, and malformed or failed observation remains fatal.
...
The ordinary package contains the real manifest-verified Vite `web/` tree and serves it identically through foreground and service-owned paths. On supported current-user Linux/systemd, service install verifies and publishes an immutable release, writes the non-secret owner-only environment, installs stable command/unit/manifest anchors, reloads the user manager, enables only `hostdeck.service`, and deliberately leaves both units stopped. Lifecycle status is read-only; start and restart require manager plus loopback API readiness; HostDeck-only restart and active upgrade preserve the independently owned Codex process/socket. Successful upgrades retain active plus immediate-previous releases. Uninstall proves the complete owned filesystem and manager identity before stopping HostDeck then Codex, disabling HostDeck, removing exact anchors/releases and `service.env`, reloading to `not-found`, and retaining only the empty installer root and lock; it does not require the API, Codex executable, network, or Tailscale and never changes Serve/profile state. `IFC-V1-058` proves the same path in a clean pinned-Noble ordinary-user environment. The selected installed workflow is documented in `docs/delivery/08-user-guide.md`; the aggregate clean packaged remote-phone release smoke remains `REL-V1-006`.
