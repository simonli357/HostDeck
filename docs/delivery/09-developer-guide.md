...
| Production package build | `pnpm build` | Offline frozen-lock build of `dist/hostdeck` from the exact 614-source server/CLI closure plus a fresh pinned Vite 8.1.4 dashboard build. Emits six compiled packages, production dependencies, real `web/` assets and strict web manifest, one `codexdeck` executable, one non-executable `dist/service-host.js`, the pure systemd unit generator, schema-4 package identity, and the dependency-free verifier. |
...
The ordinary package now contains the real manifest-verified Vite `web/` tree and serves it identically through foreground and service-owned paths. Persistent user-unit installation/lifecycle and clean-machine parity remain incomplete: the package contains the verified pure two-unit generator, but does not write an environment file, install, enable, or manage units. Do not publish service-wrapper user instructions until `IFC-V1-056` to `IFC-V1-058` close those paths.
