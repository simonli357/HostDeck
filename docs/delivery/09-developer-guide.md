...
| Production package build | `pnpm build` | Offline frozen-lock build of `dist/hostdeck` from the exact 603-source server/CLI closure. Emits six compiled library packages plus production dependencies, identity manifest, and dependency-free verifier; it intentionally emits no web assets or HostDeck executable. |
...
The required source grammar is complete in `packages/cli/src/`. Host `status` and paginated session `list` now use least-authority loopback GET routes; confirmed `revoke` uses the audited local-admin device-revoke route with exact operation/device correlation. Human status output omits the private remote origin, and human session-list output omits cwd, thread identity, objective, and summary; `--json` preserves the selected session-list contract. Paginated `devices` remains an explicit side-effect-free non-success until `IFC-V1-085` implements its secure read-only local path under `DEC-024`; `serve` and reserved service actions remain similarly staged for their owning leaves. Existing pair, lock, unlock, remote, and session/control commands use selected HTTP routes, while only `legacy status/reset` currently enters local SQLite administration. `pnpm build` compiles the accepted roots into `dist/hostdeck`, but the package still has no `bin`, shebang, startup entrypoint, web assets, or runnable `codexdeck` command.
