...
| API/CLI | Headless handlers, custom Node listener, source-level CLI shell/tests, exact probed Fastify/Zod/SSE/static dependencies, completed Tailscale 1.98.8 behavior spike, bounded read-only observer, and normalized remote contracts. | Selected local and remote stack foundations, not a packaged production path. | Ownership-safe Serve manager, exact proxy trust, production composition, full app auth, timeouts, service units, runnable `bin`. |
...
6. Read-back must prove the expected private HTTPS-to-loopback mapping and empty Funnel state before remote state becomes ready. Exact standard identity headers and forwarding cardinality are handled separately; surviving browser-supplied `X-Tailscale-*` lookalikes are untrusted. App pairing, CSRF, exact Host/Origin, and route authorization remain required; tailnet membership alone grants no HostDeck permission.
7. When the active profile changes, Tailscale stops, or Serve state drifts, HostDeck marks remote access unavailable, audits the transition, and leaves local runtime, storage, and loopback access running.
8. HostDeck never switches profiles automatically and never repairs or disables Serve state on an unrecognized/company profile. Returning to the dedicated profile triggers observation only: exact persisted state becomes ready; missing or drifted state requires explicit local enable.
9. Remote disable first invalidates HostDeck remote admission and persists disabled intent, then removes only the exactly matching HostDeck-owned Serve mapping and verifies removal. Ambiguous or failed cleanup remains visible and cannot reopen admission or be reported as successful cleanup.
