...
| Web | Structured phone fixtures, selected Focus Rail targets, a real React/Vite two-route phone shell, and an exact typed/bounded 34-route JSON client with loopback plus admitted-Serve evidence. | Shell and JSON transport foundations implemented. | SSE, CSRF/state coordination, real screens/actions, packaged assets, responsive/accessibility/fidelity, and device evidence. |
