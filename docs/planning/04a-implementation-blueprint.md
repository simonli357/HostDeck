...
| API/CLI | Headless handlers, custom Node listener, source-level CLI shell/tests, exact probed Fastify/Zod/SSE/static dependencies, completed Tailscale 1.98.8 behavior spike, bounded read-only observer, ownership-safe Serve manager, and normalized remote contracts. | Selected local and remote stack foundations, not a packaged production path. | Exact proxy trust, production composition, full app auth, timeouts, service units, runnable `bin`. |
