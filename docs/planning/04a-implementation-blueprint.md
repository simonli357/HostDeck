...
| `PendingTurnSettings` | Exact session/thread, catalog identity, model/effort and/or Plan mode, positive revision, baseline read-back, dispatch phase, accepted turn id, and bounded conflict/unknown cause. | contracts/server ephemeral control |
...
- Plan/Default selection stores revisioned pending next-turn state and builds `turn/start.collaborationMode` from the exact catalog mask. If model and Plan are both pending, the dispatcher embeds the selected model/effort into `collaborationMode.settings`; it must not send contradictory top-level fields that Codex ignores. Both revisions settle from the same settings event/read-back. There is no zero-turn Plan toggle or slash fallback.
- A paused goal can be edited/read without work. Setting/resuming `active` is an agentic mutation that can autonomously start turns; it rejects while unapplied model or Plan state exists because the goal method cannot carry those next-turn settings. Pause does not imply interrupt. Materialization markers never use active status.
