...
| Web | Structured phone fixtures, selected Focus Rail targets, a real React/Vite two-route phone shell, and exact bounded 34-route JSON, selected-route SSE, and page-memory CSRF clients. | Shell plus JSON/SSE/CSRF transport and authority foundations implemented. | Access-first host/session coordination is active under `FE-V1-025`; real screens/actions, packaged assets, responsive/accessibility/fidelity, and device evidence remain downstream. |
