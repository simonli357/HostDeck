...
| Web | Structured phone fixtures, selected Focus Rail targets, a real React/Vite two-route phone shell, exact bounded JSON/SSE/CSRF clients, the access-first host/session coordinator, and production Mission Control. | Shell, transport, page-memory authority, coordinated browser state, runtime ownership, and the default grouped inventory route are implemented. | Session Detail, actions, access recovery, real packaged assets, complete-dashboard responsive/accessibility/fidelity, and device evidence remain downstream until their owning leaves close. |
