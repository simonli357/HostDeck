...
| API/CLI | Selected Fastify/API/SSE/static composition, app auth and bounded command clients, exact Tailscale observation/Serve/proxy boundaries, accepted foreground lifecycle, and one verified compiled `codexdeck` process entry. | Runnable packaged foreground foundation with test-owned assets. | Real Vite assets, user units/service lifecycle, clean install, and interface hardening. |
