...
| Storage | Historical repositories plus selected mapping/projection/event/compatibility/recovery, durable remote-ingress configuration/observation, versioned remote enable/disable audit with preserved LAN/certificate history, and phased owner-only path/daemon-lease startup. | Remote configuration and audit durability implemented; block reopened only for aggregate hardening. | Focused remote storage hardening; production consumers remain owned by Block 04. |
...
| API/CLI | Headless handlers, custom Node listener, source-level CLI shell/tests, exact probed Fastify/Zod/SSE/static dependencies, completed Tailscale 1.98.8 behavior spike, and normalized remote contracts. | Selected local and remote stack foundations, not a packaged production path. | Read-only observer, ownership-safe Serve manager, exact proxy trust, production composition, full app auth, timeouts, service units, runnable `bin`. |
