...
| Web | Structured phone fixtures, selected Focus Rail targets, and a real React/Vite two-route phone shell with strict session navigation, accessible Host/access sheet, loading/error boundaries, component tests, and 390 x 844 browser captures. | UI foundation implemented. | Typed HTTP/SSE/CSRF/state clients, real screens/actions, packaged assets, responsive/accessibility/fidelity, and device evidence. |
...
2. Emit ESM JavaScript and declarations for only the 610-module server/CLI closure with source and declaration maps disabled.
