...
| Storage | Historical repositories plus selected mapping/projection/event/compatibility/recovery, durable remote-ingress configuration/observation, and phased owner-only path/daemon-lease startup. | Remote configuration durability implemented; block reopened for remote audit and aggregate hardening. | Remote enable/disable audit migration, production adoption, and focused hardening. |
