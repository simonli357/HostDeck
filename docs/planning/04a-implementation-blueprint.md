...
| API/CLI | Headless handlers, custom Node listener, source-level CLI shell/tests, exact probed Fastify/Zod/SSE/static dependencies, and the completed Tailscale 1.98.8 profile/Serve/proxy/SSE/phone contract. | Selected local and remote stack foundations, not a packaged production path. | Normalized remote contracts, read-only observer, ownership-safe Serve manager, exact proxy trust, production composition, full app auth, timeouts, service units, runnable `bin`. |
