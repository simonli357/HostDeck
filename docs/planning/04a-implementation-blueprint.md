...
- One stateful decoder assigns a monotonic connection-local sequence, rejects clock regression, and validates managed thread/turn/item lifecycle order. Exact repeated goal-state snapshots emitted by read operations become bounded content-free redundant observations and do not reach projection or control observers. Stable transition turn/item/goal/request identities remain retained until bounded capacity exhaustion; capacity, true transition duplicate, malformed, post-archive, and order failures latch the decoder stopped.
