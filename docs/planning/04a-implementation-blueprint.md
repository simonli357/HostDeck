...
| --- | --- | --- | --- |
| Workspace/conventions | Pinned eight-package workspace, strict TypeScript, Biome, Vitest, planning/scaffold/export/binding checks, deterministic offline package, and exact command verifier. | Reusable selected build/package foundation. | Clean install/service/release proof. |
...
| API/CLI | Selected Fastify/API/SSE/static composition, app auth and bounded command clients, exact Tailscale observation/Serve/proxy boundaries, accepted foreground/service lifecycles, one verified compiled `codexdeck` entry, and exact generated systemd user units. | Runnable packaged two-process foundation with test-owned assets and runtime-only user-manager evidence. | Real Vite assets, persistent service lifecycle/install, clean parity, and interface hardening. |
| Web | Structured phone fixtures, selected Focus Rail targets, a real React/Vite two-route phone shell, exact bounded JSON/SSE/CSRF clients, the access-first host/session coordinator, production Mission Control, and structured Session Detail. | Shell, transport, page-memory authority, coordinated browser state, runtime ownership, and the grouped inventory plus bounded replay-to-live detail routes are implemented. | Actions, access recovery, real packaged assets, complete-dashboard responsive/accessibility/fidelity, and device evidence remain downstream until their owning leaves close. |
...
| Browser JSON client | `browserHttpRouteContracts` plus `createBrowserHttpClient` bind all 34 selected JSON rows to executable request/response schemas, exact statuses, fixed paths, and route-specific CSRF policy. | Production web code imports no server module. Only the current canonical loopback HTTP or private Tailscale HTTPS origin constructs; requests are root-relative and same-origin. One outer deadline, request/response byte bounds, capacity ceiling, fatal UTF-8/JSON/Zod parsing, cancellation, stable errors, and no automatic retry apply to fetch and body consumption. |
| Browser SSE client | `createBrowserSseClient`, exact selected root-relative stream path, same-origin fetch, `eventsource-parser` 3.1.0, one in-memory per-connection cursor, immutable state/event ports, and browser-SSE resource limits. | Native `EventSource` hidden reconnect and invisible comments are not selected. Exact heartbeat comments, media/error envelope, fatal UTF-8, parser/event bytes, id/type/session/schema/contiguous cursor, first boundary, connect/idle/backoff/cap, consumer, abort/route/unmount, reader, timer, and capacity behavior stay explicit. |
...
| Listener lifecycle | One composition owner for register/ready/listen/readiness/drain/close and reverse-order startup rollback; production binds explicit loopback HTTP only. | No wildcard, assigned-private-IP, or public bind; listen requires either admitted runtime readiness or proven mutation-closed diagnostic readiness; close is idempotent and attempts every bounded cleanup even when an earlier close step fails. |
...
The browser JSON client consumes a narrow browser-resource contract while the server registry owns generic `http_response_max_bytes`. Cross-field validation requires browser/CLI response capacity to cover selected server output, browser request bytes/concurrency to fit server ceilings, and the browser deadline to cover the server route deadline. Request data and injected response ports are snapshotted before use; body chunks are streamed under the cap, cancelled/released on rejection, zeroed after assembly, and never attached to public errors. Pairing-fragment orchestration, raw in-memory CSRF generation, SSE cursor/reconnect state, and coordinated host diagnosis remain outside this client.

The implemented browser SSE client in `packages/web/src/sse-client.ts` uses fetch streaming because native `EventSource` hides the selected server's comment heartbeats, owns automatic reconnect, and does not expose pre-stream HTTP error envelopes. Exact `eventsource-parser` 3.1.0 handles the general SSE grammar under a hard buffer; HostDeck rejects parser retry directives and non-selected comments, then validates one exact id/event/data projection against session, type, schema, and contiguous cursor. Only a first-event replay boundary may jump and it remains visible. Connect/idle timers, capped read reconnect from the committed cursor, duplicate/session capacity, synchronous consumer commit order, explicit route/unmount/client cancellation, reader release, and privacy-safe terminal state are complete under `FE-V1-023`; durable storage and coordinated host diagnosis remain downstream.

The browser CSRF lifecycle is a headless page-memory authority owner over the bounded JSON client. Explicit bootstrap clears prior authority before the rotate request because response loss may hide a committed server rotation; concurrent calls are single-flight. Pairing/bootstrap adoption selects the greatest coherent generation, protected requests receive the credential internally, and epoch invalidation aborts stale work. Public state exposes generation/time but never the token. Permission/read-only/conflict failures clear authority conservatively, while no failed mutation starts a bootstrap or retry. The downstream coordinator supplies proven access/profile/revoke invalidation reasons; this client does not diagnose Tailscale or render recovery UI.

`FE-V1-025` composes those clients without weakening their owners. It reads unauthenticated access state before any protected host/session disclosure, checks access/host/session origin and authority consistency, retains only same-target authorized data as stale, purges it on access loss, derives one write gate including lock/local health/CSRF, and owns detail-stream cancellation by route/session epoch. Local loopback browser reads remain `loopback_read` under `DEC-024`; no live UI state contains proxy source keys, invented device labels, fabricated detailed compatibility, or a Mission Control stream. Exact criteria are in `artifacts/fe-v1-025-shell-connection-state-coordinator.md`.

`FE-V1-012` extends only the coordinator's detail-stream start policy. Existing callers remain live-only after the detail projection's committed cursor; Session Detail explicitly requests recent history from `null` for an empty window or `max(0, last_event_cursor - 100)` for a non-empty window. The selected SSE source still owns the atomic durable-replay-to-live handoff and reconnect cursor. The browser keeps at most 100 raw events, consolidates agent deltas by item identity, suppresses and purges them on authority loss, and never adds event-page races, arbitrary cursors, polling, durable browser history, or action ownership. Exact criteria are in `artifacts/fe-v1-012-structured-session-detail.md`.

`FE-V1-013` starts one fragment-safe app-startup owner before React and route loading. No-fragment startup creates the normal production coordinator; a successful one-attempt claim adopts its in-memory CSRF result into that same coordinator before any target read and pauses on a sanitized paired confirmation. Claim or CSRF uncertainty never replays the code. The production Host & access sheet projects only coordinator-backed origin, permission, expiry, lock, read/write, host, and applicable stream truth, while device management, CSRF recovery, lock actions, detailed remote recovery, and compatibility remain downstream. Exact criteria are in `artifacts/fe-v1-013-pairing-host-access.md`.

`IFC-V1-050` applies that contract to every protocol-bearing selected route. Request-facing services receive the exact Fastify `OperationDeadline`, check it between local/nested stages, and protocol clients derive one same-signal `timeout_ms` at each final broker call as the lesser of the operation cap and current remainder. Proven no-send timeout is failed `operation_timeout`; possible-send mutation timeout is incomplete/unknown `operation_timeout`; reads return 504 without automatic retry. Compact/approval/interrupt waits and serialized request queues remove their owners on abort, while matching late events may reconcile projections without a second response or rewritten audit success. Exact scope and evidence are frozen in `artifacts/ifc-v1-050-end-to-end-deadline-cancellation.md`.

`IFC-V1-051` freezes the matching source-CLI boundary across 15 client factories, 23 public operations, and 23 distinct selected manifest route ids. Exercising each operation once makes 25 requests because pairing alone performs status/issue/status. One direct-loopback transport owns process-local capacity, connect/whole-request/response-idle timing, caller abort, request/response wire-byte limits, strict framing/UTF-8/JSON parsing, stable stage-aware failures, and complete cleanup. One shared response reader retains operation-local canonical API messages and exact success validation; no mutation retries, alternate URL/transport, raw cause output, or remote-state/local-daemon ambiguity is allowed. Exact criteria are in `artifacts/ifc-v1-051-cli-loopback-transport-bounds.md`.
...
2. Emit ESM JavaScript and declarations for only the 611-module server/CLI closure with source and declaration maps disabled.
...
This foundation exposes compiled library entrypoints only. `FE-V1-010` provides the approved Vite source shell; `IFC-V1-053` adds its completed production dashboard assets to the package. `IFC-V1-054` adds exactly one compiled `codexdeck` bin over the source grammar: direct execution remains distinct from side-effect-free library import, help/version stay daemon-independent, foreground mode derives package assets without cwd/dev fallback and invokes the accepted foreground owner once, and the package verifier binds the bin target/shebang/mode/content. `IFC-V1-086` adds one separate compiled service-host module that is inert on import, directly runnable only through an absolute Node invocation, non-executable, absent from `bin`, and independently bound by the package manifest/verifier. `IFC-V1-055` adds the strict pure exact two-unit generator and runtime-only real-manager proof without installing files. Later leaves own persistent installation, environment-file reserved-variable policy, lifecycle/status commands, uninstall, clean Ubuntu parity, and module/release hardening.
...
12. Mark the listener ready. Mark local HostDeck ready only for the exact admitted path, then observe the active Tailscale profile and exact HostDeck-owned Serve state without mutation. Only an explicit local `remote enable` or `remote disable` may invoke the ownership-safe manager.
13. Report remote ready or a bounded remote-unavailable reason independently of local readiness. Every fatal local failure after lease acquisition closes mutable resources and releases the lease in reverse order.
...
The user-unit generator consumes canonical Node/Codex/package/environment-file inputs and the verified schema-3 service-host identity without writing files or contacting the manager. It emits exactly one static Codex unit and one `default.target` HostDeck unit. Only Codex owns `RuntimeDirectory=hostdeck`; HostDeck has only `Wants=` and `After=` toward Codex. Both use `Type=exec`, fixed bounded restart/start-limit/stop policy, `UMask=0077`, control-group termination, and journald. Generated paths are systemd-quoted and specifier-safe, and each mode-`0644` descriptor carries a deterministic content hash. Installation, environment-file ownership, daemon reload/enable, upgrade, lifecycle commands, and uninstall remain later leaves.
