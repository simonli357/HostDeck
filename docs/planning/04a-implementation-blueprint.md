...
The separate browser SSE client uses fetch streaming because native `EventSource` hides the selected server's comment heartbeats, owns automatic reconnect, and does not expose pre-stream HTTP error envelopes. Exact `eventsource-parser` 3.1.0 handles the general SSE grammar under a hard buffer; HostDeck rejects parser retry directives and non-selected comments, then validates one exact id/event/data projection against session, type, schema, and contiguous cursor. Only a first-event replay boundary may jump and it remains visible. Connect/idle timers, capped read reconnect from the committed cursor, duplicate/session capacity, synchronous consumer commit order, explicit route/unmount/client cancellation, reader release, and privacy-safe terminal state are owned by `FE-V1-023`; durable storage and coordinated host diagnosis remain downstream.
