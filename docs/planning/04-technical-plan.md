...
| UI | Exact React 19.2.8, React Router 8.2.0, Radix Dialog 1.1.20, Lucide React 1.25.0, Vite 8.1.4, `zod` 4.4.3, `eventsource-parser` 3.1.0, and `@playwright/test` 1.61.1 after the visual-selection gate. | The Focus Rail two-route phone shell, exact 34-route bounded JSON client, and bounded selected-route SSE client are implemented. The dependency-free MIT SSE parser exposes comment heartbeats and a hard parser buffer while HostDeck owns same-origin fetch, cursor validation, idle/capped reconnect, cancellation, and state. In-memory CSRF coordination, real screens/actions, packaged assets, broad responsive/accessibility coverage, and final fidelity remain downstream. |
