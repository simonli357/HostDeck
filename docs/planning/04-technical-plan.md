...
| UI | React after the visual-selection gate, with exact Vite 8.1.4 and `@playwright/test` 1.61.1. | Vite and Playwright currently prove the real browser pairing/history boundary; React screens, built assets, and screenshot fidelity remain downstream. |
...
`@hostdeck/contracts` owns one strict flat `resourceBudgetSchema` with 78 integer limits: HTTP (13), SSE (11), admission/rate/concurrency (13), Codex protocol/control (32), lifecycle (3), and CLI (6). Every entry records a unit, minimum/default/maximum, owner, breach code/action, and `hostdeck.resource.<key>` observation name. Missing fields resolve only to reviewed defaults; unknown, zero, fractional, non-finite, contradictory, or above-maximum values fail before production startup side effects.
...
A dependency-free schema-3 verifier rejects runtime, manifest, command/bin/shebang/mode/content, service-host, native-module, or link drift before load. Package acceptance copies the tree to an unrelated path, makes it read-only, imports all six roots plus the inert service host without a TypeScript loader, proves the 22-registration/35-route descriptor, and exercises five command layouts. Exact process smokes separately prove direct foreground ownership and external service ownership, including app-server replacement while HostDeck remains alive and HostDeck replacement while the sibling PID/socket survive. Real Vite assets, user units, install lifecycle, and clean-machine parity remain `IFC-V1-053` and `IFC-V1-055` to `IFC-V1-058`.
...
The CLI installs/removes versioned user-unit files and verifies them. `hostdeck-codex.service` alone owns the systemd runtime-directory lifecycle; `hostdeck.service` observes that canonical owner-only directory and socket without creating, repairing, or removing them. The HostDeck unit weakly starts/orders after the Codex unit but does not use stop/restart propagation, and neither unit depends on `tailscaled`. The CLI does not edit arbitrary user units. `systemctl --user` failures are actionable, and foreground mode remains available.
