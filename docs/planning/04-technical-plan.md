...
| Pairing/token | `POST /api/pair/claim` or equivalent token claim; optional `GET /api/pair/status`. | Short-lived local pairing code; no ambient write permission. | Expired/used/revoked codes fail explicitly and are audited. |
...
| Token claimed | Browser submits code and receives a device token/cookie for read/write mode. | Store hashed token, client label if available, created/last-used time, permission mode, revoked flag, and audit `pair_claimed`. |
| Trusted write | Prompt/slash/stop/raw requests require valid token, write permission, unlocked host, allowed action, writable session, and successful audit preflight. | Audit action type, session id, client id, bounded payload summary, result. |
...
- Auth/secrets: Pairing creates a short-lived local code and a longer-lived device token stored as a hash/metadata record. Dashboard receives an HttpOnly same-origin cookie or equivalent local token. Revoked/expired/locked states reject writes.
