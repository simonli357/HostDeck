...
| Host API | Exact `fastify` 5.10.0, `cookie` 1.1.1, and `zod` 4.4.3 with HostDeck-owned local type-provider/validator/serializer compilers. | Lifecycle hooks, limits, injection, stable errors, and controlled shutdown replace the current ad hoc listener without pulling Swagger/OpenAPI peers into V1. |
...
| Static dashboard | Exact `@fastify/static` 10.1.2 with a validated asset root, explicit browser routes, and deny-by-policy dotfile/path filtering. | Hashed asset caching and MIME/HEAD behavior are reused without allowing API-to-HTML fallback or implicit index exposure. |
...
| Windows process ownership | Exact `koffi` 3.1.4 bindings over Windows Job Objects plus the shared native file-lock port. | One current-user HostDeck owner starts fixed-argv Codex, proves authenticated readiness, rotates ephemeral authority, and kills only its assigned process tree on stop or owner-process death. |
...
| Remote ingress | Supported system Tailscale client plus private Tailscale Serve HTTPS on one human-selected saved personal profile. | One shared platform adapter accepts only `/usr/bin/tailscale` on Linux or `C:\Program Files\Tailscale\tailscale.exe` on Windows after read-only native-binary inspection, then owns fixed argv and bounded direct execution. Cross-network NAT traversal and trusted `.ts.net` HTTPS remain delegated to Tailscale while HostDeck stays loopback-only; the current exact version is 1.98.8. |
...
`@hostdeck/contracts` owns one strict flat `resourceBudgetSchema` with 99 integer limits: HTTP (14), SSE (11), admission/rate/concurrency (14), Codex protocol/control (34), lifecycle (3), remote observation (5), CLI (6), browser HTTP (4), and browser SSE (8). Every entry records a unit, minimum/default/maximum, owner, breach code/action, and `hostdeck.resource.<key>` observation name. Missing fields resolve only to reviewed defaults; unknown, zero, fractional, non-finite, contradictory, or above-maximum values fail before production startup side effects. The `hostdeck.resource.<key>` names are a declared vocabulary, not a live telemetry feed: V1 emits five real breach counters (aborted, in-flight, header-count-rejected, overload-rejected, timed-out requests) through the foreground serve snapshot, and the remaining keys carry no runtime counter. Emitting the full set is deferred under `IFC-V1-112`.
...
- Node 22 HTTP limits/timeouts: `https://nodejs.org/download/release/v22.22.2/docs/api/http.html`
- Node 22 AbortController/AbortSignal: `https://nodejs.org/download/release/v22.22.2/docs/api/globals.html`
