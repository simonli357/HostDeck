...
| Output capture | tmux `pipe-pane` logs, bounded `capture-pane` snapshots, direct PTY stream | Use `DEC-017`: live ingestion through `pipe-pane`, startup/restart recovery through bounded `capture-pane`; reject direct PTY ownership for V1. |
...
| --- | --- | --- | --- | --- |
| SPK-ARCH-001 | Can tmux `pipe-pane` or an equivalent mechanism provide ordered, reconnectable per-session output for V1? | Prototyped fake Codex output through tmux, live pipe capture, reader restart, bounded capture recovery, and missing-target failures. | Resolved by `DEC-017` and `artifacts/int-v1-001-tmux-capture-spike.md`: use `pipe-pane` for live ingestion plus bounded `capture-pane` recovery and replay boundaries when continuity cannot be proven. | Output ingestion tasks, stream API tasks. |
...
| Is tmux `pipe-pane` required for V1 streaming? | Required, optional optimization, capture-pane polling only | Resolved in `DEC-017`: `pipe-pane` is the required live ingestion path; `capture-pane` is the bounded startup/restart recovery path. | `INT-V1-001`. |
