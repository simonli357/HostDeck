...
| Host access, QR pairing, CSRF reload, devices, lock, remote profile/Serve, and compatibility UI | `FE-V1-013`, `FE-V1-031` to `FE-V1-035` | `FE-V1-013` is in progress under frozen `PHA-01` to `PHA-20` startup, disclosure, Host/access, Focus Rail, and physical-phone criteria; downstream access leaves remain ordered behind it. |
