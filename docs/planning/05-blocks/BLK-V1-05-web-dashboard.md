...
| Phone-first shell and typed HTTP/SSE/CSRF/state clients | `FE-V1-010`, `FE-V1-019`, `FE-V1-023` to `FE-V1-025` | `FE-V1-010` and exact bounded 34-route JSON client `FE-V1-019` are done; strict `FE-V1-023` browser SSE criteria are frozen and implementation is active, while `FE-V1-024` CSRF remains ready before the shared coordinator. |
