...
| --- | --- | --- | --- | --- | --- | --- | --- |
| BLK-V1-01 | Contracts, core model, and fixtures | Yes | `BLK-V1-01-contracts-core-fixtures.md` | `FR-002`, `FR-006` to `FR-009`, `FR-015`, `NFR-003`, `NFR-005` to `NFR-007`, `SFR-005`, `SFR-011` | Approved requirements, architecture, and test plan | Contracts/core/fixtures backlog group | Review |
| BLK-V1-02 | Local state, auth, audit, and config | Yes | `BLK-V1-02-local-state-auth-audit.md` | `DR-001` to `DR-010`, `SFR-001`, `SFR-002`, `SFR-004`, `SFR-006` to `SFR-008`, `PR-009` | `BLK-V1-01`, `SPK-ARCH-002`, `SPK-ARCH-003`, `SPK-ARCH-004` | Storage/auth/audit backlog group | Review |
| BLK-V1-03 | Tmux session lifecycle and output ingestion | Yes | `BLK-V1-03-tmux-output.md` | `FR-001`, `FR-003` to `FR-005`, `FR-013`, `FR-014`, `NFR-002`, `PR-001`, `PR-006`, `SFR-010` | `BLK-V1-01`, `BLK-V1-02`, `SPK-ARCH-001`, `SPK-ARCH-004` | Tmux/output backlog group | Review |
| BLK-V1-04 | Local API and CLI control plane | Yes | `BLK-V1-04-api-cli-control-plane.md` | `FR-006` to `FR-008`, `FR-011`, `FR-012`, `FR-015`, `PR-002` to `PR-004`, `PR-007`, `PR-008`, `SFR-003`, `SFR-005` | `BLK-V1-01` to `BLK-V1-03`, `SPK-ARCH-003` | API/CLI backlog group | Review |
| BLK-V1-05 | Web dashboard UX | Yes | `BLK-V1-05-web-dashboard.md` | `FR-005` to `FR-010`, `IR-001` to `IR-009`, `PR-005`, `SFR-001` to `SFR-003`, `SFR-009` | `BLK-V1-01`, `BLK-V1-04`, `SPK-ARCH-003`, `SPK-UX-001` | Web/dashboard backlog group | Review |
| BLK-V1-06 | Hardening, setup, and release readiness | Yes | `BLK-V1-06-hardening-release.md` | `NFR-001` to `NFR-009`, `PR-001` to `PR-009`, release gates | `BLK-V1-01` to `BLK-V1-05` | Hardening/release backlog group | Review |
...
| --- | --- | --- | --- | --- | --- | --- |
| BLK-V1-01 | Stable typed contracts, core state model, errors, write eligibility, and Codex-like fixtures exist before adapters/UI. | Pending backlog decomposition; must include scaffold, core model, schemas, fixtures, classifier tests, and block hardening leaf tasks. | Unit and contract test artifacts for models, schemas, errors, write gates, and fixture classification. | Fixture review proving unknown/failure states are not treated as healthy success. | Blocks all implementation work that consumes contracts. | Not started |
| BLK-V1-02 | Durable local state, pairing/token trust, lock/LAN settings, audit records, retention, and config rules work locally. | Pending backlog decomposition; must include SQLite spike, migrations, repositories, auth, audit, retention, config, restart, and hardening tasks. | Storage/auth/audit integration tests, migration tests, retention tests, restart persistence tests. | Local state inspection for hashed tokens, bounded audit payloads, visible lock/LAN behavior. | Enables safe writes, restart truth, and release privacy posture. | Not started |
| BLK-V1-03 | HostDeck can manage tmux-backed Codex sessions, ingest ordered output, reconnect streams, attach locally, and reconcile stale sessions. | Pending backlog decomposition; must include tmux spike, fake adapter, real adapter, output reader, restart reconciliation, attach, stop, and hardening tasks. | Fake adapter tests, output ordering/replay tests, stale write rejection tests. | Ubuntu tmux smoke with multiple sessions, attach, send, stop, restart, and stale target behavior. | Enables the core host-agent workflow and real-process evidence. | Not started |
| BLK-V1-04 | Typed local API and `codexdeck` CLI expose host status, sessions, writes, pairing state, lock/LAN state, and service modes with loud failures. | Pending backlog decomposition; must include route contracts, write pipeline, CLI commands, startup checks, LAN controls, service modes, and hardening tasks. | API/CLI contract tests, startup/config tests, write-ordering tests, negative failure tests. | Foreground/service-mode smoke, daemon-unavailable CLI behavior, localhost/LAN network check. | Owns user/operator control plane and command reference updates. | Not started |
| BLK-V1-05 | Phone-responsive dashboard delivers Mission Control, Session Detail, trust/safety states, prompt/slash controls, and raw fallback against approved mockups. | Pending backlog decomposition; must include UI state fixtures, visual mockup spike, screen groups, accessibility, screenshots, UI-fidelity, and hardening tasks. | Component/state tests, UI integration tests, accessibility checks, disabled-write control tests. | Human-selected mockup comparison, phone/desktop screenshots, drift notes, failure-state inspection. | Owns visible V1 user experience and UI-fidelity release evidence. | Not started |
| BLK-V1-06 | V1 can be installed, run, validated, documented, and judged go/no-go from a clean Ubuntu workflow. | Pending backlog decomposition; must include aggregate validation, setup docs, command reference, smoke scripts, security/privacy review, release checklist, and release-readiness tasks. | Full typecheck/lint/test/build aggregate, release smoke script, setup checks. | Clean Ubuntu install/run, service start/stop/status, support docs review, go/no-go checklist. | Owns release readiness, handoff truth, and known-gap visibility. | Not started |
