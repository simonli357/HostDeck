...
| BLK-V1-04 | Typed local API and `codexdeck` CLI expose host status, sessions, writes, pairing state, lock/LAN state, and service modes with loud failures. | `IFC-V1-001` to `IFC-V1-014`, `IFC-V1-090` in `api-cli-control-plane.md`. | Startup readiness evidence in `artifacts/ifc-v1-001-startup-readiness.md`; read-route evidence in `artifacts/ifc-v1-002-read-routes.md`; pairing/security route evidence in `artifacts/ifc-v1-005-security-routes.md`; remaining API/CLI contract tests, write-ordering tests, and negative failure tests are planned. | Real tmux smoke still passes after read-route work; foreground/service-mode smoke, daemon-unavailable CLI behavior, localhost/LAN network check remain planned. | Owns user/operator control plane and command reference updates. | In progress: startup readiness, read routes, and pairing/security route foundation done; streams, writes, CLI, network smoke, and hardening remain planned |
