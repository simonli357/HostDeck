...
| BLK-V1-03 | HostDeck can manage tmux-backed Codex sessions, ingest ordered output, reconnect streams, attach locally, and reconcile stale sessions. | `INT-V1-001`, `INT-V1-010` to `INT-V1-016`, `INT-V1-090` in `tmux-output.md`. | Fake adapter evidence in `artifacts/int-v1-010-fake-tmux-adapter.md`; output ordering/replay and stale write rejection tests remain planned. | Ubuntu tmux smoke with multiple sessions, attach, send, stop, restart, and stale target behavior remains blocked by `tmux` availability. | Enables the core host-agent workflow and real-process evidence. | In progress: fake adapter done; real tmux work blocked by `tmux` availability |
