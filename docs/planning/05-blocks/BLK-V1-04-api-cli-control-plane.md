...
| HTTP/SSE/idempotency/deadline/CLI bounds and stress aggregate | `IFC-V1-047` to `IFC-V1-052` | HTTP limits, bounded SSE, and idempotency/concurrency are complete in `IFC-V1-047` to `IFC-V1-049`; `IFC-V1-050` is in progress with `DLC-01` to `DLC-10` frozen across every request/service/protocol outcome boundary, CLI bounds remain ready independently, and the aggregate remains blocked behind both. |
| Deterministic production-output foundation | `IFC-V1-021` | Blocked only by the remaining deadline/CLI/resource aggregate `IFC-V1-050` to `IFC-V1-052`; selected runtime, stream, composition, and static production boundaries are accepted. |
...
| Reopened interface hardening | `IFC-V1-091` | Blocked by remaining selected interface leaves `IFC-V1-052` and `IFC-V1-058`; selected composition, legacy-interface isolation, and remote acceptance pass, and direct-LAN evidence is not a gate. |
