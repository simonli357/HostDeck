...
| Cookie auth, security audit executor, CSRF, pair/device/revoke/lock/LAN routes, and security matrix | `IFC-V1-026` to `IFC-V1-033`, `IFC-V1-059` | `IFC-V1-026` is complete with strict lazy cookie auth, local-admin provenance, manifest enforcement, real storage/raw-listener, and privacy evidence. `IFC-V1-032` audit-executor hardening is active; device listing and four independent read-route leaves are newly ready while remaining leaves stay dependency-ordered. |
