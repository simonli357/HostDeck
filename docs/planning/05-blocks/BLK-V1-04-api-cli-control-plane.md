...
| Cookie auth, security audit executor, CSRF, pair/device/revoke/lock, and reusable security foundation | `IFC-V1-026` to `IFC-V1-032`, `IFC-V1-059` | Complete for the reusable app-auth boundary: pairing/claim, CSRF, device list/revoke, lock, and audit execution pass. Selected remote provenance/rate/pair-link adaptation and aggregate proof remain in `IFC-V1-074`, `IFC-V1-077`, and `IFC-V1-079`. |
...
| Tailscale contract spike and remote ingress path | `IFC-V1-070` to `IFC-V1-079` | `IFC-V1-070` freezes exact 1.98.8 behavior, `IFC-V1-071` completes bounded read-only observation, `IFC-V1-072` completes ownership-safe private Serve mutation, `IFC-V1-073` completes exact external-origin/proxy/source trust, and `IFC-V1-075` completes the 35-route rebaseline. Remote app authorization, CLI/API composition, QR pairing, lifecycle, hostile matrix, and production physical acceptance remain. Evidence: `artifacts/ifc-v1-070-tailscale-remote-ingress-spike.md`, `artifacts/ifc-v1-071-tailscale-observer.md`, `artifacts/ifc-v1-072-tailscale-serve-manager.md`, `artifacts/ifc-v1-073-tailscale-serve-proxy-trust.md`. |
