...
| Tailscale contract spike and remote ingress path | `IFC-V1-070` to `IFC-V1-079` | `IFC-V1-070` complete for exact 1.98.8 profile/Serve/proxy/SSE/phone behavior and cleanup; observer, Serve ownership, proxy trust, source/rate composition, route rebaseline, remote CLI/API, QR pairing, lifecycle, hostile matrix, and production physical acceptance remain. Evidence: `artifacts/ifc-v1-070-tailscale-remote-ingress-spike.md`. |
...
| Exact route-manifest contract | `IFC-V1-019` | Complete for the 36-route pre-remote manifest; strict ownership remains reusable, while `IFC-V1-075` replaces direct-LAN/certificate entries before production composition. |
