...
| Tailscale contract spike and remote ingress path | `IFC-V1-070` to `IFC-V1-079` | `IFC-V1-070` freezes exact 1.98.8 behavior, `IFC-V1-071` completes bounded read-only observation, `IFC-V1-072` completes ownership-safe private Serve mutation, and `IFC-V1-075` completes the 35-route rebaseline. Proxy/source trust, remote CLI/API composition, QR pairing, lifecycle, hostile matrix, and production physical acceptance remain. Evidence: `artifacts/ifc-v1-070-tailscale-remote-ingress-spike.md`, `artifacts/ifc-v1-071-tailscale-observer.md`, `artifacts/ifc-v1-072-tailscale-serve-manager.md`. |
