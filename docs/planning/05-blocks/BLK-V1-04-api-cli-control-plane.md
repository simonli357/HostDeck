...
| Typed app, SSE adapter, static boundary, and listener lifecycle | `IFC-V1-022` to `IFC-V1-025`, `IFC-V1-036`, `IFC-V1-037`, `IFC-V1-078` | Complete through mutable health, application drain, and production remote lifecycle; typed app, bounded SSE, hardened static boundary, upfront runtime ownership, exact loopback listener, Node limit inventory, and bounded reverse cleanup pass. Full selected route composition remains `IFC-V1-046`. |
...
| Replay/live, subscriber bounds, mutable health, shutdown, and aggregate stream matrix | `IFC-V1-034` to `IFC-V1-038`, `IFC-V1-078`, `IFC-V1-079` | Headless replay-to-live continuity, sustained global/device/session-bounded selected SSE subscribers, independent seven-source local/remote health, shutdown, production remote lifecycle, and aggregate physical acceptance are complete. Full selected product-route/SSE registration remains in `IFC-V1-046`. |
...
| Host/session/event reads, start/resume/archive, prompt, per-control, approval, interrupt, and selected API/CLI aggregate | `IFC-V1-039` to `IFC-V1-046`, `IFC-V1-060` to `IFC-V1-065`, `IFC-V1-068`, `IFC-V1-069` | Every individual selected route and CLI leaf is complete; `IFC-V1-046` is ready to assemble and prove them together through the production loopback and admitted remote boundaries. |
| Legacy custom-listener and raw/tmux route disposition | `IFC-V1-067` | In progress: `ISO-01` to `ISO-08` freeze selected-only package roots, loopback HTTP listener, loopback/remote authority, CLI HTTP-only administration, reviewed LAN-settings retirement, historical data preservation, coverage replacement, and residue evidence before production packaging. |
...
| Reopened interface hardening | `IFC-V1-091` | Blocked by remaining selected interface leaves `IFC-V1-046`, `IFC-V1-052`, `IFC-V1-058`, and `IFC-V1-067`; selected remote acceptance now passes, and direct-LAN evidence is not a gate. |
