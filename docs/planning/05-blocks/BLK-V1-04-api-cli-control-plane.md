...
| Replay/live, subscriber bounds, mutable health, shutdown, and aggregate stream matrix | `IFC-V1-034` to `IFC-V1-038` | Headless replay-to-live continuity and sustained global/device/session-bounded selected SSE subscribers are complete with retention, overflow, revoke, disconnect, archive, source-failure, and real paused-response evidence. Mutable health, production lifecycle composition, shutdown, and aggregate acceptance remain. |
