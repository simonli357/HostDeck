...
| Replay/live, subscriber bounds, mutable health, shutdown, and aggregate stream matrix | `IFC-V1-034` to `IFC-V1-038` | Headless replay-to-live continuity, sustained global/device/session-bounded selected SSE subscribers, and independent seven-source local/remote health with startup maintenance are complete. Shutdown, production lifecycle composition, and aggregate acceptance remain. |
