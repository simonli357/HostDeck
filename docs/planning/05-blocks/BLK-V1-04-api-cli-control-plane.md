...
| HTTP/SSE/idempotency/deadline/CLI bounds and stress aggregate | `IFC-V1-047` to `IFC-V1-052` | HTTP limits, bounded SSE, and idempotency/concurrency are complete in `IFC-V1-047` to `IFC-V1-049`; deadline propagation and CLI transport bounds are ready independently, and their aggregate remains blocked behind both. |
