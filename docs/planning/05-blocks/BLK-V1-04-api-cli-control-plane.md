...
| Cookie auth, security audit executor, CSRF, pair/device/revoke/lock, and reusable security foundation | `IFC-V1-026` to `IFC-V1-032`, `IFC-V1-059`, `IFC-V1-074` | Complete through selected remote authorization: admitted-source limits, pairing, device authority, CSRF, lock, revoke, generation currentness, cookie publication, and real private Serve composition pass without identity authority or LAN fallback. Fragment-safe pair-link/browser startup and aggregate physical proof remain `IFC-V1-077` and `IFC-V1-079`. |
...
| Tailscale contract spike and remote ingress path | `IFC-V1-070` to `IFC-V1-079` | `IFC-V1-070` freezes exact 1.98.8 behavior; `IFC-V1-071` to `IFC-V1-073` complete observation, ownership-safe Serve mutation, and proxy/source trust; `IFC-V1-074` completes pairing and application authorization; `IFC-V1-075` completes the 35-route rebaseline. Remote CLI/API controls, QR/browser pairing, lifecycle/SSE, hostile matrix, and production physical acceptance remain `IFC-V1-076` to `IFC-V1-079`. Evidence: `artifacts/ifc-v1-070-tailscale-remote-ingress-spike.md`, `artifacts/ifc-v1-071-tailscale-observer.md`, `artifacts/ifc-v1-072-tailscale-serve-manager.md`, `artifacts/ifc-v1-073-tailscale-serve-proxy-trust.md`, `artifacts/ifc-v1-074-tailscale-serve-authorization.md`. |
