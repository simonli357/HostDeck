...
| FR-019 | The local CLI must discover eligible persisted root Codex CLI threads through the selected app-server using bounded metadata, excluding archived, ephemeral, child/subagent, incompatible, already managed, missing, or invalid-cwd threads. | Must | Adapter/API/CLI eligibility, pagination, malformed-response, privacy, and exact-runtime tests. |
...
| PR-006 | V1 supports new HostDeck-managed threads and non-destructive local adoption of eligible persisted root Codex CLI threads. Both paths preserve the exact Codex thread for HostDeck control and shared-runtime TUI resume; arbitrary terminal-process import, transcript copying, and concurrent takeover remain deferred. | Must | Start/discover/adopt/unmanage/resume plus rejection and exact coexistence tests. |
