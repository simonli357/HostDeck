...
| FR-011 | The V1 CLI must provide runnable `serve`, `status`, `start`, `discover`, `adopt`, `unmanage`, `list`, `send`, `resume`, `interrupt`, `archive`, `pair`, `devices`, `revoke`, `lock`, `unlock`, `remote enable`, `remote status`, `remote disable`, and `service` operations. | Must | Packaged CLI contract tests and clean-install command smoke. |
| FR-012 | The local HostDeck API must provide host/runtime status, session start/discover/adopt/unmanage/list/detail/events/stream, prompt, structured controls, approval response, interrupt/archive, pairing/device/security, lock, and read-only remote-ingress state; unmanaged-thread discovery/adoption/unmanage, profile switching, and remote-ingress mutation remain explicit local CLI operations. | Must | Route manifest, schema, auth, failure, and integration tests. |
...
| FR-019 | The local CLI must discover eligible persisted top-level user-facing Codex CLI threads through the selected app-server using bounded metadata, including ordinary top-level fork conversations while excluding archived, ephemeral, parent-linked/subagent, incompatible, already managed, missing, or invalid-cwd threads. | Must | Adapter/API/CLI eligibility, pagination, malformed-response, privacy, and exact-runtime tests. |
| FR-020 | The local CLI must adopt exactly one eligible persisted Codex thread by stable id and unique HostDeck alias without copying or rewriting Codex history, bootstrap only bounded recent projection with an explicit adoption boundary, and resume it through HostDeck's dedicated app-server. | Must | Transaction, race, restart, event handoff, prompt, phone-list, and exact TUI coexistence tests. |
| FR-021 | The local CLI must unmanage exactly one quiet HostDeck session without archiving, deleting, renaming, forking, or otherwise mutating the underlying Codex thread. | Must | Storage/API/CLI tests plus Codex read-back before and after unmanage. |
...
| NFR-009 | After supported Codex and system Tailscale clients are installed, HostDeck V1 runs as a normal Ubuntu or Windows user without HostDeck administrator/root privileges, router changes, or a HostDeck public/privileged listener. | Must | Clean Ubuntu and Windows prerequisite/setup/service smoke. |
...
| NFR-014 | Public release artifacts are versioned, signed where the platform supports install-time trust, accompanied by SHA-256 checksums, SBOM, and build provenance, and contain no build-host path or secret. | Must | Native release workflow, signature, provenance, SBOM, artifact-content, and tamper tests. |
...
| DR-010 | Audit storage is durable and bounded by count/age with explicit types for pair, claim, revoke, lock/unlock, remote-ingress enable/disable, session start/adopt/unmanage, prompt/control, approval, interrupt, and archive; historical LAN/certificate rows remain readable but are not selected V1 actions. | Must | Migration, retention, restart, and type-coverage tests. |
...
| ID | Requirement | Priority | Validation |
| --- | --- | --- | --- |
| PR-001 | V1 supports Ubuntu 24.04 x64 and Windows 11 x64 with a locally authenticated, supported Codex CLI; tmux is not a product runtime requirement. | Must | Native clean-host compatibility and real-Codex smokes. |
| PR-002 | HostDeck and Codex app-server bind only private local transports: loopback HTTP for HostDeck, a Unix socket on Linux, and authenticated loopback WebSocket on Windows. The separately installed Tailscale process may expose private Serve HTTPS, but HostDeck opens no LAN/public listener. | Must | Native process/listener/endpoint ownership and inventory tests. |
...
| PR-006 | V1 supports new HostDeck-managed threads and non-destructive local adoption of eligible persisted top-level user-facing Codex CLI threads, including ordinary top-level forks. Both paths preserve the exact Codex thread and fork provenance for HostDeck control and shared-runtime TUI resume; arbitrary terminal-process import, transcript copying, child/subagent adoption, and concurrent takeover remain deferred. | Must | Start/discover/adopt/unmanage/resume plus rejection and exact coexistence tests. |
...
| PR-008 | Foreground development and unprivileged per-user long-running modes share runtime contracts and documented lifecycle commands; Linux uses systemd user units and Windows uses an interactive-user startup agent rather than a Session 0 service. | Must | Native foreground/background parity smoke. |
...
| PR-010 | App-server communicates only through a current-user local transport and is never the browser-facing trust boundary; Windows authentication tokens are passed only through protected configuration/environment boundaries and never command arguments or output. | Must | Socket/WebSocket/listener/secret inspection and architecture test. |
...
| PR-012 | V1 produces native runnable `codexdeck` packages with built web assets and installable/upgradable/uninstallable per-user lifecycle definitions for each supported host. | Must | Native clean build/package/install/upgrade/uninstall smoke. |
...
| PR-014 | Linux and Windows packages preserve user state/config across compatible upgrades, support deterministic rollback to the previous retained release, and remove only HostDeck-owned program/lifecycle files on uninstall unless data removal is separately confirmed. | Must | Native upgrade/failure-injection/rollback/repeated-uninstall matrix. |
| PR-015 | Windows public distribution uses a signed per-user MSIX with Start Menu/CLI entry points and update metadata; unsigned portable output is internal validation evidence only. Linux public distribution uses a versioned native archive/package with an exact manifest and signature/checksum policy. | Must | Signature/install identity, update metadata, and artifact-policy tests. |
| PR-016 | Release artifacts are built and tested on pinned native Ubuntu and Windows CI runners; cross-compilation alone cannot satisfy native-module, path-security, lifecycle, or install evidence. | Must | CI matrix and native evidence manifests. |
| PR-017 | Tailscale discovery, profile observation, Serve ownership, enable/disable, and noninterference contracts behave equivalently through validated platform command adapters on Ubuntu and Windows. | Must | Native Tailscale CLI contract, profile/Serve, and cleanup smokes. |
...
| FR-011 | `BLK-V1-04`, `BLK-V1-06` | `IFC-V1-040` to `IFC-V1-045`, `IFC-V1-051`, `IFC-V1-054`, `IFC-V1-058` to `IFC-V1-065`, `IFC-V1-075`, `IFC-V1-076`, `IFC-V1-080`, `IFC-V1-084`, `IFC-V1-085`, `REL-V1-003`, `REL-V1-006` | Packaged CLI, remote command, and clean-install matrix. |
...
| FR-017 | `BLK-V1-01`, `BLK-V1-03`, `BLK-V1-04` | `FND-V1-015`, `INT-V1-003`, `INT-V1-006`, `INT-V1-021`, `INT-V1-027`, `INT-V1-091`, `IFC-V1-087` | Schema/version compatibility matrix plus production binary observation and diagnostic admission. |
| FR-018 | `BLK-V1-03`, `BLK-V1-04`, `BLK-V1-06` | `INT-V1-007`, `INT-V1-028` to `INT-V1-032`, `IFC-V1-025`, `IFC-V1-037`, `IFC-V1-055`, `IFC-V1-058`, `IFC-V1-071`, `IFC-V1-072`, `IFC-V1-076`, `IFC-V1-078`, `IFC-V1-086`, `REL-V1-006` | Process/listener/Tailscale ownership and service smoke. |
| FR-019 | `BLK-V1-01`, `BLK-V1-03`, `BLK-V1-04` | `INT-V1-106`, `FND-V1-102`, `INT-V1-107`, `IFC-V1-110`, `INT-V1-109` | Exact-runtime discovery semantics, normalized eligibility, local CLI/API, and interoperability hardening. |
| FR-020 | `BLK-V1-01` to `BLK-V1-04` | `INT-V1-106`, `FND-V1-102`, `DAT-V1-105`, `INT-V1-107`, `INT-V1-108`, `IFC-V1-110`, `INT-V1-109` | Exact identity/history bootstrap, atomic state, activation/restart, local control plane, and real same-thread evidence. |
| FR-021 | `BLK-V1-01` to `BLK-V1-04` | `FND-V1-102`, `DAT-V1-105`, `INT-V1-108`, `IFC-V1-110`, `INT-V1-109` | Non-destructive unmanage contracts, atomic storage, service/API/CLI, and Codex read-back. |
...
| NFR-009 | `BLK-V1-04`, `BLK-V1-06`, `BLK-V1-07` | `IFC-V1-021`, `IFC-V1-053` to `IFC-V1-058`, `IFC-V1-086`, `IFC-V1-103` to `IFC-V1-109`, `REL-V1-104`, `REL-V1-105` | Native normal-user install/Tailscale-prerequisite/lifecycle smoke. |
...
| NFR-012 | `BLK-V1-03`, `BLK-V1-04`, `BLK-V1-06` | `INT-V1-003`, `INT-V1-006`, `INT-V1-021`, `INT-V1-028`, `INT-V1-091`, `IFC-V1-087`, `REL-V1-006` | Codex compatibility artifact and production version-drift evidence. |
...
| NFR-014 | `BLK-V1-06`, `BLK-V1-07` | `IFC-V1-101` to `IFC-V1-108`, `REL-V1-102` to `REL-V1-106`, `REL-V1-108` | Native signature/checksum/SBOM/provenance/tamper and release evidence. |
...
| DR-010 | `BLK-V1-02`, `BLK-V1-04` | `DAT-V1-023`, `DAT-V1-024`, `DAT-V1-027`, `DAT-V1-030`, `DAT-V1-032`, `DAT-V1-105`, `IFC-V1-032`, `IFC-V1-075`, `IFC-V1-076`, `IFC-V1-110` | Audit type/migration/outcome/retention/restart tests. |
| DR-011 | `BLK-V1-02`, `BLK-V1-03` | `DAT-V1-018`, `INT-V1-003` | Compatibility persistence tests. |
| PR-001 | `BLK-V1-03`, `BLK-V1-06`, `BLK-V1-07` | `INT-V1-003`, `INT-V1-006`, `INT-V1-027`, `INT-V1-032`, `INT-V1-100` to `INT-V1-105`, `REL-V1-104`, `REL-V1-105` | Ubuntu/Windows native real-Codex smoke. |
| PR-002 | `BLK-V1-04`, `BLK-V1-06`, `BLK-V1-07` | `IFC-V1-017`, `IFC-V1-025`, `IFC-V1-058`, `INT-V1-101`, `INT-V1-102`, `IFC-V1-109`, `REL-V1-104`, `REL-V1-105` | Native loopback/socket/authenticated-WebSocket/Tailscale listener inventory. |
...
| PR-006 | `BLK-V1-01` to `BLK-V1-05` | `INT-V1-005`, `INT-V1-031`, `INT-V1-106` to `INT-V1-109`, `FND-V1-102`, `DAT-V1-105`, `IFC-V1-040`, `IFC-V1-060`, `IFC-V1-061`, `IFC-V1-110`, `FE-V1-038` | New start plus native discover/adopt/unmanage, exact shared TUI resume, and deferred-boundary rejection. |
...
| PR-008 | `BLK-V1-04`, `BLK-V1-06`, `BLK-V1-07` | `IFC-V1-055` to `IFC-V1-058`, `IFC-V1-086`, `IFC-V1-105`, `IFC-V1-107`, `REL-V1-104`, `REL-V1-105` | Native foreground/per-user background parity smoke. |
...
| PR-010 | `BLK-V1-03`, `BLK-V1-04`, `BLK-V1-07` | `INT-V1-004`, `INT-V1-007`, `INT-V1-031`, `INT-V1-032`, `INT-V1-100` to `INT-V1-103`, `IFC-V1-025` | Private socket/WebSocket token and listener inspection. |
| PR-011 | `BLK-V1-05`, `BLK-V1-06` | `FE-V1-040`, `REL-V1-007` | Browser matrix. |
| PR-012 | `BLK-V1-04`, `BLK-V1-06`, `BLK-V1-07` | `IFC-V1-021`, `IFC-V1-053` to `IFC-V1-058`, `IFC-V1-086`, `IFC-V1-101` to `IFC-V1-108`, `REL-V1-104`, `REL-V1-105` | Native build/package/install/upgrade/uninstall smoke. |
| PR-013 | `BLK-V1-07` | `IFC-V1-101` to `IFC-V1-104`, `REL-V1-104`, `REL-V1-105` | Bundled runtime/native dependency inventory and clean-host invocation. |
| PR-014 | `BLK-V1-06`, `BLK-V1-07` | `DAT-V1-103`, `DAT-V1-104`, `IFC-V1-105`, `IFC-V1-107`, `IFC-V1-108`, `REL-V1-104`, `REL-V1-105` | Native upgrade/rollback/retention/uninstall failure matrix. |
| PR-015 | `BLK-V1-07` | `IFC-V1-103`, `IFC-V1-104`, `IFC-V1-106`, `IFC-V1-107`, `REL-V1-103` to `REL-V1-105` | Linux artifact and Windows signed-MSIX policy evidence. |
| PR-016 | `BLK-V1-07` | `REL-V1-101`, `IFC-V1-102`, `REL-V1-104`, `REL-V1-105` | Native CI/build/package/clean-host evidence. |
| PR-017 | `BLK-V1-04`, `BLK-V1-07` | `IFC-V1-100`, `IFC-V1-109`, `REL-V1-104`, `REL-V1-105` | Native Tailscale profile/Serve/noninterference parity. |
| PR-018 | `BLK-V1-06`, `BLK-V1-07` | `REL-V1-102`, `REL-V1-103`, `REL-V1-106`, `REL-V1-108` | Fail-closed tag/version/signing/publication evidence. |
