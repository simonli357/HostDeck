...
| `pnpm smoke:codex-shared-sessions` | `INT-V1-113` | Prove loaded-before and created-after enrollment, plain `codex` socket use, bounded history, reconnect, and cleanup. | Pass against exact reviewed Codex 0.147.0 with no discover/adopt/handoff command. |
| `pnpm test:shared-runtime-hardening` | `INT-V1-114` | Bind the fixed hostile shared-runtime inventory and one real headless SideCue/MarketPilot/ScandyControl-style start/resume/reconnect/crash run to a clean commit. | `SRH-01` to `SRH-12` pass once without retry, fallback, content retention, desktop-window launch, or owned residue. |
...
`IFC-V1-056` lifecycle evidence must meet `SLC-01` to `SLC-24`. Temporary-home tests exercise a full verified source/staged/current package, strict schema-1 owner and environment manifests, no-follow ownership/collision checks, advisory-lock contention, write-ahead transaction recovery, exact fixed-argv bounded manager behavior, install/enable-without-start, every lifecycle postcondition, read-only status, active/inactive semantic upgrade, same-release idempotence, downgrade/same-version drift rejection, and failure injection before and after every publication/manager stage. Upgrade rollback must restore the prior selector and readiness while preserving Codex PID/socket and byte-identical config/state/database/Tailscale fixtures. A real user-manager smoke must prove installed anchor/file/mode/hash inventory, exact enablement, local API readiness rather than active-only inference, repeated commands, HostDeck-only restart, complete stop, rollback, failed-unit preservation, privacy, and zero process/listener/descriptor/staging residue.

`IFC-V1-057` uninstall evidence must meet `UNS-01` to `UNS-24`: exact ownership and preservation preflight, six forward-only journal phases, HostDeck-before-Codex stop, HostDeck-only disable, anchor/release/environment removal, reload-to-not-found proof, absent/repeated/recovered behavior, reinstall, active-plus-previous retention, tamper refusal, private output, real-user-manager inspection, Tailscale noninterference, and zero owned residue.

`IFC-V1-058` clean-environment evidence must meet `CEP-01` to `CEP-24`. Direct tests own the pinned manifest, fixed-argument host runner, source/image/UID/capability identity, bounds, redaction, evidence schema, Tailscale snapshot comparison, and failure cleanup. One no-retry real run must clone the exact commit into an empty UID-1000 home in a pinned Ubuntu 24.04 userspace with systemd as PID 1 and a real user manager; run the documented frozen install; build and verify deterministic versioned packages; compare compiled foreground and installed-service API/dashboard behavior; inspect exact processes, cgroups, socket, listeners, files, and units; prove independent HostDeck/Codex restart, Tailscale-unavailable local readiness, observation-only host-profile stability, active upgrade/retention, active and repeated uninstall, sentinel preservation, and complete teardown. The artifact must identify the shared-kernel Docker limit and cannot satisfy the later independent release/phone smoke.
...
| Host/access | Remote status checking, disabled, Tailscale unavailable, local host status with laptop profile mismatch, Serve missing/ready/foreign/conflict/drift/public, invalid external origin, unpaired, QR/link claim, read-only, writer, reload/CSRF bootstrap, expired/revoked, locked, profile switch while connected, and generic browser/network failure when the phone cannot reach the origin. Checking is browser-owned and never claims that a local enable or Serve mutation was accepted. |
...
`FE-V1-040` owns exact package-level browser portability. Its executable ledger maps all 39 interactions and 12 journeys; four pinned Chromium/Firefox phone/desktop projects run the same 19 scenarios over the compiled package boundary. The matrix covers navigation/history, fragment pairing, SSE continuity, prompt and structured controls, utilities, approvals, lifecycle/security actions, recovery, responsive/input semantics, diagnostics, and an isolated real cross-site `SameSite=Strict` check. Strict reports record package/browser/project identity, bounded counts, observations, limitations, and cleanup without raw credentials or machine identity. Physical Android, Firefox Android, live Serve trust/routing, Safari/iOS, visual-diff closure, and clean-package release aggregation remain separate owners.
