...
| `pnpm test:package` | `IFC-V1-021`, `IFC-V1-054` | Build twice, inspect and verify the production tree, then run unrelated-path read-only import/native/loopback lifecycle, five-layout command invocation, command/verifier mutation, rollback, and failure-injection checks. | Pass on the exact supported Node/pnpm Linux build target without network or source TypeScript execution. |
