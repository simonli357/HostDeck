...
| `INT-V1-003`, `DAT-V1-018`, `IFC-V1-039`, `IFC-V1-082`, `IFC-V1-083`, `IFC-V1-086` | `IFC-V1-087`, then `FE-V1-035` and `IFC-V1-091` | Real binary observation, durable incompatibility/session truth, diagnostic-ready startup, and a strict public projection must exist before compatibility UI or interface hardening. |
| `DAT-V1-019`, `DAT-V1-025`, `IFC-V1-024`, `IFC-V1-025`, `IFC-V1-038`, `IFC-V1-046`, `IFC-V1-052`, `IFC-V1-067` | `IFC-V1-021`, `IFC-V1-053` to `IFC-V1-058`, and `IFC-V1-080` to `IFC-V1-086` are complete | Deterministic outputs include the selected grammar, API/local commands, resources, application, foreground/service lifecycles, executable, exact user units, manifest-verified dashboard assets, persistent lifecycle, safe uninstall, and clean supported-user parity. |
| Completed `IFC-V1-058`, `IFC-V1-079`, and `IFC-V1-087` | Completed `IFC-V1-091` production interface hardening | Clean packaged parity, remote physical proof, and production compatibility diagnostics are accepted inputs; aggregate module hardening passes and formal matrix closure remains `REL-V1-008`. |
...
| Completed `FE-V1-003`, `IFC-V1-046`, `FE-V1-010`, bounded JSON/SSE clients `FE-V1-019`/`FE-V1-023`, in-memory CSRF `FE-V1-024`, coordinator `FE-V1-025`, Mission Control `FE-V1-011`, structured Session Detail `FE-V1-012`, and fragment-safe pairing/host access `FE-V1-013` | Ready `FE-V1-014`, `FE-V1-020` to `FE-V1-022`, and `FE-V1-026` to `FE-V1-034`, `FE-V1-036` to `FE-V1-038`; `FE-V1-035` waits for `IFC-V1-087` | Proven inventory, detail, pairing, access, and shared-client foundations support remaining UI leaves; compatibility UI additionally requires producible backend truth. |
...
| Complete UI states/actions | Completed `FE-V1-016`, `FE-V1-039`, `FE-V1-040`, `FE-V1-017`, and `FE-V1-018`; delegated pre-phone leaves `FE-V1-091` to `FE-V1-098`; then guard `FE-V1-099`, independent review `FE-V1-100`, and aggregate `FE-V1-090` | Responsive, accessibility, browser, fidelity, and copy are complete. Exact physical-driver boundaries and one consolidated review now precede the next no-retry phone candidate and aggregate module closure. |
