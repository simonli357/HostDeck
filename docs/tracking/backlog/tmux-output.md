...
| `INT-V1-011` | blocked | `BLK-V1-03`, `FR-001`, `FR-014`, `DR-007`, `PR-006` | Ubuntu with tmux | `tmux` binary unavailable in current environment | `INT-V1-012`, `INT-V1-015` | Implement real tmux target naming, target lookup, and list/reconcile primitives. | Targets are deterministic enough for restart reconciliation and do not import arbitrary terminals. | Fake adapter interface is available from `INT-V1-010`; real adapter tests require `tmux` availability. |
