...
| `BLK-V1-03` Codex runtime/events | Exact binding, private IPC/handshake, corrected no-model thread lifecycle, ordered projection, prompt/model/goal/Plan controls, generation-aware structured usage, strict real deny/approve/expiry routing, and exact active-turn interrupt pass; tmux mechanics are legacy. | Compact, skills, assembled callback/runtime composition, command-sandbox reproducibility, supervision, restart acceptance, and legacy disposition remain. | `INT-V1-005` to `INT-V1-008`, `INT-V1-017` to `INT-V1-032`, `INT-V1-091` |
