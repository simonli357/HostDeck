...
- Release state: no-go; deterministic server/CLI output, real manifest-verified dashboard assets, accepted foreground/service processes, exact runtime-proven user units, and the complete V1 dashboard behavior exist, but persistent service lifecycle/install parity, remaining browser/fidelity/copy/device acceptance, interface hardening, and release hardening remain incomplete.
- Next exit: `IFC-V1-056` proves persistent per-user install, upgrade, status, lifecycle, and rollback against exact systemd units before uninstall and clean-machine parity.
...
| Build/package/user services | In progress | `IFC-V1-021` deterministic server/CLI output, `IFC-V1-053` real manifest-verified dashboard assets, `IFC-V1-054` executable dispatch, `IFC-V1-086` packaged service process, and `IFC-V1-055` exact user units are complete. Persistent lifecycle/install parity remains `IFC-V1-056` to `IFC-V1-058`. |
| Mobile visual selection/UI/device | In progress | Visual contracts/selection, complete dashboard screens/actions/trust/recovery states, responsive layout, semantic accessibility, packaged assets, focused phone evidence, and exact Chromium/Firefox phone/desktop package coverage are complete. Visual fidelity, copy/workflow review, aggregate phone module acceptance, and release acceptance remain. |
