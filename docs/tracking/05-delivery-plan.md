...
| `BLK-V1-02` State/auth/audit | Mapping/recovery/projection/runtime compatibility, remote-ingress config/profile/Serve/observation durability, retention, pairing/CSRF/device authority, append-only audit, secure paths/lease, and aggregate restart/privacy evidence pass. | Remote enable/disable audit migration and focused remote storage hardening remain. | `DAT-V1-032`, `DAT-V1-092` |
