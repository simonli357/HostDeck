...
- Current pass: Storage-owned local state/auth/audit, tmux fake adapter foundation, tmux output-capture spike, real tmux target primitives, real managed start/send/stop/attach/output/restart/smoke/hardening, pairing/security API route foundation, and validation wiring complete; API/CLI startup is next
...
| `BLK-V1-04` API / CLI Control Plane | In progress | Startup, sessions, streams, write dispatch/audit ordering, CLI, service controls, network smoke, and hardening remain planned; startup is ready next. | `artifacts/ifc-v1-005-security-routes.md` |
