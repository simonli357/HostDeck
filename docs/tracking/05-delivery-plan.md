...
- Release state: no-go; deterministic server/CLI output, the accepted foreground listener, and independently restartable packaged foreground/service processes exist, but real dashboard assets, user units/service lifecycle, install parity, UI, and release hardening remain incomplete.
- Next exit: `IFC-V1-055` binds the proven service-owned process and external app-server ownership into exact unprivileged systemd user units.
...
| Build/package/user services | In progress | `IFC-V1-021` deterministic package, `IFC-V1-054` executable dispatch, and `IFC-V1-086` packaged service process are complete. `IFC-V1-055` user units are next; web assets and lifecycle/install parity remain `IFC-V1-053` and `IFC-V1-056` to `IFC-V1-058`. |
