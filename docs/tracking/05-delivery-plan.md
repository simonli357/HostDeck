...
- Current pass: Storage-owned local state/auth/audit, tmux fake adapter foundation, tmux output-capture spike, pairing/security API route foundation, and validation wiring complete; real tmux target implementation is next
...
| `BLK-V1-03` Tmux Session / Output | In progress | Real target naming/listing, Codex start, send/stop/attach, output reader, restart reconciliation, real smoke, and hardening remain planned. | `artifacts/int-v1-001-tmux-capture-spike.md`, `artifacts/int-v1-010-fake-tmux-adapter.md` |
