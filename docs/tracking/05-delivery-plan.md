...
- Release state: no-go; deterministic server/CLI output, real manifest-verified dashboard assets, accepted foreground/service processes, exact runtime-proven user units, persistent install/upgrade/uninstall/retention, clean supported-user parity, selected-target fidelity, and the complete V1 dashboard behavior exist, but copy/device acceptance, interface hardening, and release hardening remain incomplete.
- Next exit: `FE-V1-018` closes copy/workflow ambiguity before aggregate real-phone hardening; `IFC-V1-091` is independently ready.
