...
- Current pass: Storage-owned local state/auth/audit, tmux fake adapter foundation, and pairing/security API route foundation complete; validation wiring is next while real tmux/startup-dependent work remains blocked
...
| `BLK-V1-03` Tmux Session / Output | In progress | Real tmux spike/adapter work is blocked by `tmux` availability; output reader and restart reconciliation remain planned. | `artifacts/int-v1-010-fake-tmux-adapter.md` |
