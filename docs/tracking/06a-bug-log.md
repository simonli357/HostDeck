...
| BUG-030 | Approval confirmation action is not admitted by exact semantic, clickable, fully visible reveal on the real hierarchy. | High | Release blocker | In progress | `FE-V1-090` | Pushed `91cb32a` safely opened the exact semantic confirmation before write authority correctly closed it on a server-caused stream reconnect. Exact-action physical proof remains pending behind `BUG-034`. |
| BUG-031 | Failure cleanup can leave a background Android Chrome process after returning Home. | High | Release blocker | In progress | `FE-V1-090` | Thirteen failed executions ended with Serve absent, launcher foreground, Chrome stopped, dedicated profile selected, and clean worktree. Successful-run absence proof remains. |
...
| BUG-033 | Approval sequencing can cross a stream reconnect and mistake prompt-composer geometry for the closed confirmation footer. | High | Release blocker | In progress | `FE-V1-090` | Pushed `91cb32a` moves approval before event diagnostics, gates approval and prompt on stable write authority, and permits no anonymous action fallback. Its phone run failed closed when `BUG-034` removed authority; corrected physical proof remains. |
| BUG-034 | The finite server request deadline terminates a committed long-lived SSE response and removes phone write authority during an active workflow. | High | Release blocker | In progress | `FE-V1-090` / Fastify SSE transport | Real-listener regressions reproduce deadline loss and the pre-commit-abort race. The candidate retains the deadline through source opening, explicitly owns the committed stream, preserves disconnect cancellation and bounded iterator cleanup, and passes 63 focused cases/one intentional physical skip plus 2,918 unit, 245 contract, 36 integration, and 932 web cases; corrected phone proof remains. |
