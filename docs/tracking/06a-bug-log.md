...
| BUG-030 | Approval confirmation action is not admitted by exact semantic, clickable, fully visible reveal on the real hierarchy. | High | Release blocker | In progress | `FE-V1-090` | Clean pushed `306859b` proves explicit product ARIA still projects zero exact Android matches. Diagnose normalized/containing public labels, the Cancel peer, and focused geometry before defining a footer selector. |
...
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: the critical confirmation button relied on descendant text for its computed browser accessible name, but this is not the whole mismatch. Clean pushed `306859b` proves an explicit product `aria-label` also produces zero exact Android text or content-description matches through all four reveal states while 16 to 18 clickable nodes, including confirmation-footer geometry, remain. Xiaomi Chrome's UiAutomator projection therefore omits both descendant-derived and explicit ARIA names for this footer.
- Candidate result: retain the explicit product label and exact selector while extending only the private-free diagnostic. It now counts exact, whitespace-normalized, and containing expected labels; exact/normalized/containing public `Cancel` peer labels; and at most four focused-node geometries. No raw labels, anonymous tap, fuzzy action, or fallback is introduced.
...
- Fix: the exact semantic candidate retains unlabeled clickable hierarchy nodes and at most six changed private-free observations, while the production submit button keeps the exact accessible label. The next observation adds only public-label match classes and focused geometry needed to define a fail-closed footer relationship. Raw hierarchy, arbitrary labels, origins, profiles, accounts, pairing material, prompts, and event payloads remain excluded.
- Validation: direct driver fixtures cover exact/description controls, rejection states, normalized/containing counts, focused anonymous geometry, and target-label absence. Focused 30/one intentional physical skip, CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. The diagnostic physical run remains pending.
