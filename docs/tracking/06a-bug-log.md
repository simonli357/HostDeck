...
| BUG-039 | Full Chrome refresh retains deep Session Detail scroll while the stale-state oracle requires a top notice. | High | Release blocker | In progress | `FE-V1-090` | The stale detail request occurred, but `Showing stale session state` remained above the current hierarchy. The candidate reuses bounded dock-aware backward reveal before capture. CLI driver 32/one intentional physical skip, typecheck, and formatting pass. |
