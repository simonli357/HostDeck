...
| BUG-049 | Host evidence capture rejects the intentionally rendered private origin as if it were retained Chrome material. | High | Release blocker | In progress | `FE-V1-090` | The clean `710ef06` run matched the observed canonical inert `TextView` but confirmed Xiaomi Chrome omits the decorative owner labels from UIAutomator. The candidate instead requires unique exact embedded Host heading, back-to-actions control, and visible read/write permission context in the same app viewport before redaction; URL/edit controls and every other private shape remain fail-closed. |
