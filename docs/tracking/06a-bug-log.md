...
| BUG-036 | The physical recovery fixture injects a terminal storage error while requiring the browser to reconnect automatically. | High | Release blocker | In progress | `FE-V1-090` | Pushed `0fb3586` removes the invalid injection and holds one real reconnect request after `source_failed`. A clean run proved exact `holding/1`, one failure, zero active subscribers, and one request; `BUG-037` masked the required visible block until the proxy returned `504`. Same-request release to one restored subscriber remains pending corrected physical proof. |
| BUG-037 | Accepted prompt progress masks a later composer availability block, leaving disabled writes without a visible cause. | High | Release blocker | In progress | `FE-V1-090` | The clean pushed `0fb3586` run proved the reconnect state and blocked writes, but the composer retained `Turn completed` instead of exposing the reconnect reason. Pushed `7858b61` prioritizes non-turn availability failures, preserves accepted-turn progress semantics, and restores accepted status after recovery. Web 902, focused server/CLI/web 187 with one intentional physical skip, CLI/web typecheck, and formatting pass; corrected physical proof remains. |
...
- Current evidence: the clean `0fb3586` recovery-state failure is the eighteenth product-sequence post-mutation cleanup proof. One separate cellular preflight failure at the same candidate restored Wi-Fi, mobile data, Serve, profile, launcher, Chrome, tunnels, stay-awake, and worktree state before Chrome UI execution.
- Closed by: pending corrected physical evidence and implementation commit.
