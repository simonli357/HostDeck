...
| BUG-030 | Approval confirmation action is not admitted by exact semantic, clickable, fully visible reveal on the real hierarchy. | High | Release blocker | In progress | `FE-V1-090` | Clean pushed `6a1c8c9` proved the title appears on open but is absent with the status by action acquisition. Prevent Cancel autofocus scrolling and add unique close/risk diagnostics while retaining the action gate. |
...
### BUG-030 Approval Confirmation Uses Text-Only Identity
...
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: Xiaomi Chrome's UiAutomator projection omits both descendant-derived and explicit expected names for the confirmation footer. Clean pushed `868b15a` proves exact, normalized, containing, and focus matches stay absent while the unique bottom row consistently exposes a text/description-empty left region and text-empty/description-set right region. The clean pushed `6a1c8c9` run adds temporal evidence: the exact title appears immediately after opening, then title/status are absent when the action is acquired while the footer remains. The approval dialog explicitly focuses Cancel without preventing browser scroll, so the current candidate corrects that product-level mobile viewport risk before reconsidering any selector context.
- Candidate result: retain the explicit product label and prefer exact semantic selection. Only when semantics are absent, require unique exact title and modal status, one or two exact reason nodes with exactly one fully inside the page between that title and status, then select the unique rightmost node from exactly two aligned bottom-row clickables with the observed empty-left/opaque-described-right pattern, bounded action size, lower-page position, symmetric margins, seam, and right-quadrant rules. Cancel autofocus now uses `preventScroll`; selector enforcement remains unchanged and its private-free diagnostic adds unique close-confirmation and elevated-risk observations alongside exact/normalized/containing status projection. Wait without swiping and tap once. No opaque-value read, arbitrary nearest-node, coordinate constant, retry, or raw hierarchy is used.
- Harsh success criteria: prefer one exact clickable semantic match fully inside the Chrome page. If the target phone exposes none, permit only the frozen confirmation-specific relational context, empty/set, and geometric contract above; reject missing/duplicate title or status, zero/more-than-two reasons, zero/multiple reasons between title/status, any extra/missing/both-empty/text-named footer node, invalid page/action/margin/alignment/seam/quadrant geometry, and every ambiguity. Perform one tap with no retry and retain exact pending-request and responding-state assertions.
...
- Fix: the dedicated selector now uses unique title/status plus a geometrically modal reason, exact-first semantics, and the frozen empty-left/opaque-described-right footer relationship. A bounded waiter records only private-free counts/geometry/empty-set flags and never swipes the fixed sheet. The product keeps its explicit accessible label for browser/screen-reader semantics.
- Validation: direct fixtures cover a duplicate background reason with one modal reason, exact semantic preference, valid opaque-described right-footer selection, missing/duplicate context, missing/extra/both-empty/text-named footer nodes, normalized/containing counts, focused geometry, private-free empty-set flags, target-label absence, and exact-versus-containing status plus close/risk summaries. Component regression requires `{ preventScroll: true }` while preserving Cancel focus and origin restoration. Focused component/driver 40/one intentional physical skip, web/CLI typechecks, production web build, six approval browser scenarios, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. The clean pushed `6a1c8c9` run reports `exact=0/1/0`, no status containment, and the unchanged strict footer after the transient opening title; exact cleanup passes. Commit/push and corrected physical proof remain pending.
...
- Validation: direct regression freezes and asserts Home-before-force-stop ordering. Focused/static gates passed before the clean pushed runs; `1d37ef1`, `306859b`, `d57c523`, `868b15a`, `0e1781a`, `43b27a5`, `de5b22b`, and `6a1c8c9` each ended with Serve absent, launcher foreground, Chrome stopped, dedicated profile selected, and clean worktree. Successful-run physical absence proof remains pending.
- Closed by: pending corrected physical evidence and implementation commit.
