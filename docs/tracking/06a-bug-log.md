...
| BUG-049 | Host evidence capture rejects the intentionally rendered private origin as if it were retained Chrome material. | High | Release blocker | In progress | `FE-V1-090` | The clean `c142b9c` diagnostic proved one exact canonical full-origin `TextView`, non-clickable, description/resource-free, unregistered, and fully app-contained, with no hostname-only or second private node. The candidate admits both observed Chrome web-text classes and the exact source/rendered `Origin` or `Private address` label forms; URL/edit controls and every other private shape remain fail-closed. |
