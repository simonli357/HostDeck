...
| BUG-028 | Physical screenshot cropping rejects Chrome's valid collapsed-toolbar accessibility state. | High | Release blocker | In progress | `FE-V1-090` | After exact redacted-dialog proof on `c5925c2`, capture found one bounded compositor but no toolbar node. The selector must support explicit expanded/absent states while rejecting malformed or duplicate toolbar geometry. |
