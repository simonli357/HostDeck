...
| BUG-042 | Android Chrome omits exact child slash text from visible composite session-utility rows. | High | Release blocker | In progress | `FE-V1-090` | The clean `b9797bc` run opened and captured the approved Session utilities menu, then could not acquire exact child text `/usage`. The candidate gives each whole row an exact `Open /usage`, `Open /compact`, or `Open /skills` accessible name, retains explanatory and disabled-reason text through `aria-describedby`, and drives the explicit clickable semantics. Focused 93/one intentional physical skip, web 935, affected typechecks, Biome, and planning pass; corrected physical proof remains. |
