...
- Fix: parse and validate the active plug mask, pass it into temporary stay-awake enforcement, and retain the existing exact restoration/cleanup path. The Xiaomi path now enables bit `1`; USB-reported devices continue to enable bit `2`.
- Validation: parser regression covers AC/USB/wireless, inactive, invalid, and contradictory observations. Focused Android-driver tests pass 46 with one intentional physical skip and typecheck passes. The correction requires commit/push and one fresh clean aggregate run before this bug can close.
