...
| BUG-020 | The supported-browser oracle can inspect routed mutation failures before the intercepted request settles, making a valid synthetic response pass or fail according to scheduler timing. | High | Release blocker | Closed | `FE-V1-040` / `FE-V1-090` | Pending requests now settle before diagnostics are inspected; only the exact proven mutation path may admit Playwright's intercepted-route abort signal after terminal UI, one request, schema, and CSRF checks. Focused Chromium phone 19/19 and full matrix 76/76 pass. |
...
- Closed by: the `FE-V1-090` browser-package evidence commit and regenerated `artifacts/fe-v1-040-supported-browser-interaction-matrix/` reports.
