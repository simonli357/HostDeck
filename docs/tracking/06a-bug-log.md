...
| BUG-024 | Physical target acquisition treats an offscreen or Chrome-occluded named session row as immediately actionable. | High | Release blocker | In progress | `FE-V1-090` | The pushed `7d078bb` phone run acquired the whole named row but its one tap produced no Session Detail read. Full Chrome page-viewport containment now gates acquisition; corrected physical acceptance remains pending. |
| BUG-025 | Physical failure cleanup can issue lifecycle-owned remote disable before a fresh idle observation after restoring the dedicated profile. | High | Release blocker | In progress | `FE-V1-090` | The pushed `7d078bb` failure reached absent Serve through fallback cleanup but reported primary lifecycle-disable failure. Cleanup now waits one fresh idle cycle; corrected physical acceptance remains pending. |
...
### BUG-024 Named Session Target Is Not Proven Inside The Chrome Page

- Symptom: the pushed `7d078bb` phone run paired, completed fragment-free reload, expanded the quiet queue, and acquired the newly named whole-session row, but its single tap produced neither a Session Detail request nor an active detail subscriber before the 30-second deadline.
- Impact: semantic existence alone does not prove a native target is currently actionable. A target partly below the compositor or under Chrome UI can satisfy geometry and naming checks while its center tap misses the page.
- Route: high release blocker and small physical-driver correction inside `FE-V1-090`; no product, visual, API, authorization, ingress, or retry-contract change.
- Related requirements: `NFR-004`, `PR-005`, `MDH-05`, `MDH-06`, `MDH-07`, `MDH-15`, `MDH-20`, `MDH-21`, `MDH-24`.
- Affected / owning task: `FE-V1-090` physical Android target acquisition.
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: `revealAndroidUiNode` returned the first matching Android semantic node without comparing its bounds to the actual Chrome page region below the toolbar. Adding `aria-label` made the whole row discoverable earlier than its child text, exposing this previously hidden assumption in the below-fold quiet-session flow.
- Fix: derive one strict Chrome page viewport from the unique toolbar and compositor nodes; require the whole named session link to fit inside that region before acquisition; keep bounded forward reveal active until it does; and include sanitized bounds and known-state truth if a one-tap transition still fails.
- Validation: the realistic Android hierarchy regression distinguishes the 288-pixel whole row from its 63-pixel text fragment, accepts the row when fully within the Chrome page, and rejects the same row when it extends below the compositor. Focused driver checks pass 27 with one intentional physical skip; typecheck, Biome/package exports, planning, all 2,912 unit checks with 29 intentional skips, and unchanged deterministic 6,231-entry package acceptance pass. Corrected physical acceptance remains pending.
...
- Symptom: after the `BUG-024` assertion, cleanup reported that selected-lifecycle remote disable failed. The ownership-safe fallback subsequently removed Serve, and independent inspection proved absent Serve plus fully restored device and process state.
...
- Validation: independent post-failure inspection proved Serve `{}`, original Wi-Fi/mobile-data/stay-awake and accessibility settings, stopped Chrome, no ADB forward/reverse tunnel, no acceptance process, and no partial evidence or worktree change. Focused driver checks, typecheck, Biome/package exports, planning, all 2,912 unit checks with 29 intentional skips, and unchanged deterministic 6,231-entry package acceptance pass. Corrected physical cleanup remains pending.
- Closed by: pending corrected physical evidence and implementation commit.
