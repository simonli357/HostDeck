...
| BUG-044 | Android omits child labels from Session Actions composite rows that have no explicit whole-button name. | High | Release blocker | In progress | `FE-V1-090` | The clean `8f8b10c` run opened Session Actions but could not acquire the third `Resume on laptop` child text. The candidate gives all four rows exact `Open ...` names plus described-by detail and bounded-reveals the exact clickable Resume, Interrupt, and Archive rows. Focused 99/one intentional physical skip, full web 904, both affected typechecks, Biome, and planning pass; corrected physical proof remains. |
