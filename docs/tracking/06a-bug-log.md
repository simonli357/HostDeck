...
| BUG-046 | The physical fixture marks the session active without publishing the exact active-turn event required by Interrupt. | High | Release blocker | In progress | `FE-V1-090` | The clean `cff9f75` run refreshed the active projection and one stream, but the phone retained only the prior completed turn event; the oracle also waited for prompt-receipt copy that is not valid after reload. The candidate publishes ordered active/interrupted events for the exact interrupt turn and admits readiness only through the clickable exact Interrupt row. Corrected physical proof remains. |
