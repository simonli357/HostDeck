...
| BUG-030 | Approval confirmation looks only at Android text instead of the action's exact semantic name. | High | Release blocker | In progress | `FE-V1-090` | `30f55f4` reached the fixed confirmation footer but could not find `Approve once` by text. Exact semantic reveal must accept text or content-description encoding without fuzzy matching. |
...
- Fix: confirmation acquisition now uses exact semantic equality, which accepts either Android text or content description but no substring/fuzzy match. The reusable reveal requires one match, full Chrome-page containment, clickability, and at most four bounded forward swipes before returning the unchanged one-tap action.
- Validation: direct fixtures accept text-only and description-only `Approve once` controls and reject missing, duplicate, non-clickable, and toolbar/out-of-page controls. Focused 29/one intentional physical skip, CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. Corrected no-retry physical proof remains pending.
