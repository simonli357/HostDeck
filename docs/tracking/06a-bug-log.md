...
| BUG-038 | The post-recovery phone oracle requires top-page `Current` while Session Detail remains scrolled at the fixed composer. | High | Release blocker | In progress | `FE-V1-090` | Exact counters proved one restored subscriber and no extra request, but the offscreen label timed out. The candidate requires unique restored `Turn completed`, no reconnect/unavailable marker, one subscriber, and captures recovered UI truth. CLI driver 32/one intentional physical skip, typecheck, and formatting pass. |
