...
- Symptom: pushed candidate `8754a63` physically passed Host lock, local-only unlock, and the saved-profile-away boundary, then recovered the dedicated profile and remote-ready Host state. Capture of `fe090-39-profile-recovered.png` failed closed because the page hierarchy still contained private browser material.
...
- Blocks: corrected no-retry aggregate physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: `fe090-39-profile-recovered.png` is captured while the Host & access sheet intentionally renders the exact private Serve origin, but this single call did not request the already hardened product-origin redaction path used by the other Host checkpoints. The selector correctly refused to publish the unredacted page.
- Harsh success criteria: every screenshot taken while Host & access is open must use one mandatory Host-evidence capture owner; admit redaction only for one or two exact, inert, page-contained origin text nodes under one coherent Host title, back control, and `Read & write` context; reject a hostname-only value, description, editable/clickable/resource-owned node, missing/duplicate context, ambiguous geometry, or any registered non-origin private value. Keep the raw screenshot unpublished on rejection, retain no private diagnostic, and prove the next no-retry phone run saves the recovered-profile checkpoint before continuing.
...
- Current state: all Host-sheet checkpoints now route through one mandatory capture owner. Focused Android-driver 44 passed/one intentional physical skip, CLI typecheck, targeted Biome, and planning pass; corrected physical proof remains pending.
