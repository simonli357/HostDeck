...
| BUG-037 | Accepted prompt progress masks a later composer availability block, leaving disabled writes without a visible cause. | High | Release blocker | In progress | `FE-V1-090` | Pushed `7858b61` prioritizes non-turn availability failures while preserving accepted-turn progress. The clean `f51ff84` run rendered and captured the reconnect reason and restored one subscriber; fixed-composer accepted-result restoration remains pending behind `BUG-038`. |
| BUG-038 | The post-recovery phone oracle requires top-page `Current` while Session Detail remains scrolled at the fixed composer. | High | Release blocker | In progress | `FE-V1-090` | Exact counters proved one restored subscriber and no extra request, but the offscreen label timed out. Pushed `9b3d2c5` requires unique restored `Turn completed`, no reconnect/unavailable marker, one subscriber, and captures recovered UI truth. CLI driver 32/one intentional physical skip, typecheck, and formatting pass. |
...
- Current evidence: the clean `f51ff84` recovery-oracle failure is the nineteenth product-sequence post-mutation cleanup proof. One separate cellular preflight failure at `0fb3586` restored Wi-Fi, mobile data, Serve, profile, launcher, Chrome, tunnels, stay-awake, and worktree state before Chrome UI execution.
- Closed by: pending corrected physical evidence and implementation commit.
...
- Closed by: implementation `7858b61`; corrected physical evidence remains pending.
...
- Closed by: implementation `9b3d2c5`; corrected physical evidence remains pending.
