...
| BUG-043 | Android exposes the Skills search label as a detached tiny node and leaves the visible EditText unnamed. | High | Release blocker | In progress | `FE-V1-090` | The clean `3d46d8d` run proved one read plus current Skills truth, then recorded the hidden label at x=87..92 and visible EditText at x=171..992 with no text/description. The candidate removes the detached label and sets the exact name directly on the input; its component regression requires `aria-label="Search skills"` and zero associated label nodes. Focused 79/one intentional physical skip, full web 904, both affected typechecks, Biome, and planning pass; corrected physical proof remains. |
