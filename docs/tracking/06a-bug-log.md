...
| BUG-043 | Physical utilities acquire hierarchy-present `/skills` beneath the Chrome bottom edge. | High | Release blocker | In progress | `FE-V1-090` | Clean `888fe19` and `9f36121` runs both acquired the same `/skills` node ending at Android y=2324 and failed to enter Skills; changing the completion oracle from footer status to the content editor did not alter the boundary. The candidate bounded-forward-reveals every exact utility action until one clickable node is fully inside Chrome, then requires the Skills editor, `1 matching`, and one terminal read count. Focused 79/one intentional physical skip, CLI typecheck, Biome, and planning pass; corrected physical proof remains. |
