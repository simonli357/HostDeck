...
| BUG-030 | Approval confirmation action is not admitted by exact semantic, clickable, fully visible reveal on the real hierarchy. | High | Release blocker | In progress | `FE-V1-090` | The clean pushed `7481406` semantic candidate still timed out. Bounded match/geometry/clickability/page-state diagnostics must identify the remaining contract mismatch. |
| BUG-031 | Failure cleanup can leave a background Android Chrome process after returning Home. | High | Release blocker | In progress | `FE-V1-090` | After `7481406`, Serve and foreground state were clean but `pidof` retained Chrome until manual force-stop. Home must precede force-stop and final cleanup must verify settled absence. |
...
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: the confirmation button's stable contract is the accessible name `Approve once`, but the physical driver searches only Android's `text` attribute. Chrome accessibility may expose a named control through either text or content description.
- Candidate result: the clean pushed `7481406` run replaced text-only lookup with exact semantic, clickable, fully visible reveal and still timed out. The current generic timeout does not report whether the exact match is absent, duplicated, non-clickable, or outside the selected page, so root-cause refinement remains pending.
...
- Fix: the exact semantic candidate now retains unlabeled clickable hierarchy nodes and records at most six changed private-free reveal observations: exact match count, separate text/description counts, first-match bounds/clickability, bounded page region, eligibility, and at most six anonymous clickable-node geometries. Raw hierarchy, arbitrary labels, origins, profiles, accounts, pairing material, prompts, and event payloads remain excluded.
- Validation: direct fixtures accept text-only and description-only `Approve once` controls and reject missing, duplicate, non-clickable, and toolbar/out-of-page controls. The diagnostic fixture proves description-only admission, anonymous clickable retention, bounded geometry, and target-label absence. Focused 30/one intentional physical skip, CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. Corrected no-retry physical proof remains pending.
...
- Validation: direct regression freezes and asserts Home-before-force-stop ordering. Focused 30/one intentional physical skip, CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. Failed-run and successful-run physical absence proof remain pending.
- Closed by: pending corrected physical evidence and implementation commit.
