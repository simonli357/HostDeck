...
| BUG-040 | Returning from direct not-found navigation can show cached ready UI while no server SSE subscriber exists. | High | Release blocker | In progress | `FE-V1-090` | The clean `c3fd9a4` run rendered `Ready to send` but retained zero active subscribers. Pushed `6fa0579` reloads BFCache restoration and requires exactly one fresh detail read plus one fresh stream/subscriber before UI-ready truth. Unit 2,922/29 intentional skips, web 904, focused web/CLI 142/one intentional physical skip, root static, and planning gates pass. |
...
### BUG-040 Direct Return Can Restore Cached Ready UI Without SSE

- Symptom: the clean `c3fd9a4` run safely revealed/captured stale state, refreshed back to current, navigated directly to a missing session, and captured `Session unavailable`. Returning directly to the selected session rendered `Ready to send`, but the physical server retained zero active projection subscribers after 15 seconds.
- Impact: the phone can present stale writable-looking UI after page/history restoration without a live event source, and the aggregate run cannot safely continue into controls or mutations.
- Route: high release blocker and product browser-lifecycle correction plus stricter physical oracle inside `FE-V1-090`; the SSE retry, authorization, route, pairing, and architecture contracts remain unchanged.
...
- Root cause: production `pagehide` closes startup and its coordinator exactly once, but a document restored from browser back-forward cache had no `pageshow.persisted` recovery owner. The physical oracle then accepted cached `Ready to send` text before proving a new protected detail read or stream subscriber.
- Harsh success criteria: close page-owned coordinator authority on pagehide; ignore normal pageshow; reload exactly once on persisted pageshow; reject malformed binding inputs and support idempotent unbind; after not-found, prove zero prior active subscriber; on return require exactly one fresh detail request, one fresh stream request, one newly opened/active subscriber, then exact UI-ready truth; reject cached text, missing/duplicate requests, extra subscriber, or retry; retain private-free diagnostics and exact cleanup.
- Fix: add one tested browser page-lifecycle binding that reloads only BFCache restoration after existing close ownership. The physical return sequence now snapshots exact request/subscriber counts, requires one fresh read and stream before checking `Ready to send`, and attaches bounded stream diagnostics on failure.
- Validation: direct lifecycle tests cover normal pageshow, pagehide close, one persisted reload, repeated persisted signal suppression, idempotent unbind, partial-binding cleanup, and malformed binding rejection. Unit 2,922/29 intentional skips, web 904, five adjacent web/CLI files 142 with one intentional physical skip, root typecheck/lint/exports, and the 220-task/84-requirement/683-dependency planning graph pass. Corrected phone proof remains pending.
- Closed by: implementation `6fa0579`; corrected physical evidence remains pending.
