...
| BUG-040 | Direct Android-intent navigation can confuse retained-tab authority with same-tab history recovery. | High | Release blocker | In progress | `FE-V1-090` | Pushed `6fa0579` closes the real BFCache lifecycle gap. The clean `8b3077b` run then proved the external missing-session intent retained the original selected-session tab and its one live subscriber, invalidating the zero-subscriber/fresh-return oracle. The corrected candidate adds one missing read only, returns once with Android Back, and requires every selected-detail/stream/opened/active count to remain exact. Direct 34/one intentional physical skip, CLI typecheck, and Biome pass; corrected phone proof remains pending. |
...
- Root cause: production `pagehide` closed startup and its coordinator exactly once, but a document restored from browser back-forward cache had no `pageshow.persisted` recovery owner. Separately, physical `openChromePath` uses an external Android `VIEW` intent; the latest run proved that this opens or selects a separate Chrome tab while retaining the prior selected-session tab and stream. The former oracle incorrectly treated that multi-tab transition as same-tab navigation.
- Harsh success criteria: retain the tested pagehide/BFCache lifecycle binding. Begin direct not-found proof with exactly one active selected-session subscriber and snapshot selected-detail reads, missing-detail reads, stream requests, opened subscribers, and active subscribers. Opening the missing-session intent must add exactly one missing-detail read and change no selected-session counter or subscriber. One explicit Android Back action must restore exact `Ready to send` truth with the same one subscriber and no new selected-detail read, stream request, opened subscriber, retry, or mutation. Reject every missing, lost, duplicate, or extra count; retain private-free diagnostics and exact cleanup.
- Fix: `6fa0579` adds the tested browser lifecycle owner. The corrected physical candidate separately counts missing-detail reads, freezes all selected-session authority counters, removes the second direct selected-session intent, and returns to the retained tab with one explicit Android Back action.
- Validation: lifecycle coverage remains unit 2,922/29 intentional skips, web 904, adjacent web/CLI 142/one intentional physical skip, root static, and planning gates. The corrected driver adds exact positive and per-field drift coverage; its direct 34/one intentional physical skip, CLI typecheck, and Biome checks pass. Corrected no-retry phone proof remains pending.
