...
| BUG-047 | The physical Interrupt oracle expects `Done` to retain Session Actions although the owned UI contract closes every dialog. | High | Release blocker | In progress | `FE-V1-090` | The clean `ab7d685` run completed Interrupt and tapped `Done`; private-free diagnostics showed only Session Detail landmarks, matching component and integrated web regressions. The candidate requires the actions trigger plus writable composer to return with neither actions nor result dialog retained. Corrected physical proof remains. |
