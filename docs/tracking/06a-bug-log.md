...
| BUG-027 | Physical event diagnostics can cross a row boundary when associating a body label with the geometrically nearest details action. | High | Release blocker | In progress | `FE-V1-090` | `7027de6` admits the safe redacted row but selects an action below its notice; production order proves that action belongs to the next row. Association must require the bounded preceding header action. |
...
- Validation: the direct matrix admits the bounded preceding action while rejecting a geometrically closer following-row decoy, missing preceding action, excessive distance, top/bottom occlusion, incomplete overlays, and ambiguous label. The collapsed-toolbar regression still proves screenshot selection fails while interaction admission remains valid. Focused 29/one intentional physical skip, CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. Corrected no-retry physical proof remains pending.
