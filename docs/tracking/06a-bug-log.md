...
| BUG-030 | Approval confirmation action is not admitted by exact semantic, clickable, fully visible reveal on the real hierarchy. | High | Release blocker | In progress | `FE-V1-090` | The clean pushed `1d37ef1` diagnostic found zero exact text/description matches across four bounded states while anonymous footer clickables remained. Add an explicit product `aria-label` and retain exact selection. |
...
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: the critical confirmation button relied on descendant text for its computed browser accessible name. The clean pushed `1d37ef1` diagnostic found zero exact Android text or content-description matches through all four bounded reveal states while 16 to 18 clickable nodes, including confirmation-footer geometry, remained. Chrome therefore did not expose the descendant-derived name as a stable UiAutomator semantic node on the target phone.
- Candidate result: the product button now declares exact `aria-label="Approve once"`; the harness retains exact text-or-description equality, uniqueness, clickability, full page containment, bounded reveal, and one tap. No anonymous-geometry or fuzzy-label fallback is introduced.
- Harsh success criteria: use exact semantic matching, defined as equality against text or content description; never use substring/fuzzy matching; require at most one match, bounded forward reveal, full Chrome-page containment, clickable geometry, and one tap with no retry; fail closed on missing/duplicate/non-clickable controls; retain exact pending-request and responding-state assertions.
...
- Fix: the exact semantic candidate retains unlabeled clickable hierarchy nodes and at most six changed private-free observations, while the production submit button now declares the exact accessible label. Raw hierarchy, arbitrary labels, origins, profiles, accounts, pairing material, prompts, and event payloads remain excluded.
- Validation: direct driver fixtures accept text-only and description-only controls and reject missing, duplicate, non-clickable, and toolbar/out-of-page controls. The product component regression asserts the explicit attribute. Nine component tests, focused 30/one intentional physical skip, root plus both package typechecks, production build, all six approval browser scenarios, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. The attribute is nonvisual and introduces no Focus Rail structural drift. Corrected no-retry physical proof remains pending for this product candidate.
...
- Validation: direct regression freezes and asserts Home-before-force-stop ordering. Focused 30/one intentional physical skip, CLI typecheck, lint across 829 files, package exports, and planning passed before push. The clean pushed `1d37ef1` failed run then proved Serve absent, launcher foreground, Chrome stopped, dedicated profile selected, and clean worktree after cleanup. Successful-run physical absence proof remains pending.
- Closed by: pending corrected physical evidence and implementation commit.
