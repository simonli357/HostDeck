...
| BUG-024 | Physical Session Detail timeout conflates native target activation, detail-route read, and SSE subscription. | High | Release blocker | In progress | `FE-V1-090` | The pushed `824fbec` run proved the whole target fully visible but the combined transition still timed out. Exact clickability, current bounds, route labels, GET count, and subscriber count now diagnose the next run. |
...
### BUG-024 Physical Session Detail Transition Is Ambiguous

- Symptom: the pushed `7d078bb` phone run timed out on the combined Session Detail transition. The first correction required full Chrome page containment; the pushed `824fbec` rerun then proved a fully visible `985x275`-pixel target at `[50,1635][1035,1910]`, but the same combined transition timed out.
- Impact: the aggregate phone sequence cannot proceed beyond Mission Control, and the old error cannot identify whether Android dispatched the link, the detail GET occurred, or the detail SSE subscription failed.
- Route: high release blocker and diagnostic physical-driver correction inside `FE-V1-090` before selecting a product or harness root fix; no visual, API, authorization, ingress, or retry-contract change yet.
- Related requirements: `NFR-004`, `PR-005`, `MDH-05`, `MDH-06`, `MDH-07`, `MDH-15`, `MDH-20`, `MDH-21`, `MDH-24`.
- Affected / owning task: `FE-V1-090` physical Android target acquisition.
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Ruled out: target naming, 44 CSS-pixel geometry, and Chrome page containment are now proven. The visibility guard remains valid hardening but was not sufficient and is not claimed as the timeout root cause.
- Diagnostic fix: preserve bounded Android `clickable` state; report trigger and current semantic-node geometry; distinguish Mission Control/detail labels; require zero retained detail activity before the tap; and report exact detail-request and active-subscriber counts only after the one-tap deadline. The next result can route the root fix without a repeated ambiguous run.
- Validation: the realistic Android hierarchy regression distinguishes the 288-pixel clickable whole row from its 63-pixel non-clickable text fragment, accepts the row when fully within Chrome, and rejects it below the compositor. Focused driver checks pass 27 with one intentional physical skip; typecheck and Biome/package exports pass. The prior full 2,912-unit and unchanged deterministic package gates remain valid because this candidate changes diagnostics only. One diagnostic physical run remains pending.
