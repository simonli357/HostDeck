...
| BUG-043 | Chrome's Android UIAutomator bridge leaves the HTML Skills EditText unnamed even with a direct ARIA label. | High | Release blocker | In progress | `FE-V1-090` | The clean `f3df938` run retained current one-read truth but reported no Search semantic and one unnamed clickable page editor. The candidate retains the browser ARIA contract and admits only one page-contained editor between exact current Skills landmarks; its Xiaomi-geometry regression excludes Chrome's URL field and rejects absent landmarks, duplicates, and non-clickable controls. Focused CLI 36/one intentional physical skip, full web 904, both affected typechecks, Biome, and planning pass; corrected physical proof remains. |
