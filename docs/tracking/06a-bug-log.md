...
| BUG-031 | Failure cleanup can leave a background Android Chrome process after returning Home. | High | Release blocker | In progress | `FE-V1-090` | Twenty-five product-sequence post-mutation failed executions ended with Serve absent, Chrome stopped, dedicated profile selected, and clean worktree; one cellular preflight failure also restored exact state. The harness also proved launcher foreground before one unrelated app later interrupted the device. One separate auto-lock preflight refusal made no mutation. Successful-run absence proof remains. |
...
| BUG-041 | A visible `/plan` trigger can consume one physical tap without opening the non-mutating control sheet. | High | Release blocker | In progress | `FE-V1-090` | The clean `c2f834b` run crossed `BUG-040`, model, and goal, then one exact visible/clickable `/plan` tap produced neither current-mode UI nor mutation. The candidate uses the established bounded verified-tap protocol only for sheet entry and requires exactly one Plan read; selection remains one-shot. Focused 104/one intentional physical skip, CLI typecheck, Biome, and planning pass. |
...
### BUG-041 Physical Plan Trigger Can Consume One Tap Without Opening

- Symptom: the clean `c2f834b` run crossed the corrected not-found/BFCache sequence, completed Model and Goal controls, then found and measured one visible, clickable `/plan` trigger. One coordinate tap did not render `Plan control ready`; after 30 seconds the trigger was absent from the current hierarchy and no Plan mutation was pending.
...
- Blocks: corrected no-retry aggregate physical acceptance, current package/browser refresh, retained phone deployment, and all downstream release tasks.
- Root cause: the physical driver accepted one accessibility snapshot and dispatched one coordinate tap without verifying that the non-mutating sheet entry produced its read/UI transition. Android consumed that tap without a Plan terminal state, while the existing verified-tap owner already handles this bounded non-mutating dispatch boundary by reacquiring the same unique semantic control only when no transition occurred.
- Harsh success criteria: use only the unique exact `/plan` description; retain target measurement and full visible/clickable admission; after a swallowed first tap, reacquire at most one unique matching trigger and permit one bounded second non-mutating tap; fail on missing or duplicate trigger, ambiguous region, opened-but-nonterminal sheet, or a third tap. Require exactly one Plan GET and exact `Plan control ready`; keep radio selection and protected submit strictly one-shot with one mutation and no automatic request retry.
- Fix: count exact selected-session Plan GETs and route initial/reopen sheet entry through the established verified Android tap owner. Require one read after each successful open; leave Plan choice, submit, release, apply, and close ownership unchanged.
- Validation: focused driver plus Plan state/component 104/one intentional physical skip, CLI typecheck, Biome, and the 220-task/84-requirement/683-dependency planning graph pass. Corrected no-retry phone proof remains pending.
