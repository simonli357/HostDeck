...
| BUG-029 | Inline approval review can be hierarchy-present beneath the fixed session-control dock. | High | Release blocker | In progress | `FE-V1-090` | `ffd5a1d` acquired `Review & approve` below the dock; its one tap correctly did nothing. A reusable app-overlay-safe content reveal must own the control before tapping. |
...
- Validation: direct checks accept the safely positioned clickable review action and reject a dock-occluded action, duplicate target, and missing dock geometry. Forward and backward gestures use exact reversed coordinates inside the content region. Focused 29/one intentional physical skip, CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. Corrected no-retry physical proof remains pending.
