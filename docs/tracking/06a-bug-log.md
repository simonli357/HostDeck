...
| BUG-030 | Approval confirmation action is not admitted by exact semantic, clickable, fully visible reveal on the real hierarchy. | High | Release blocker | In progress | `FE-V1-090` | Clean pushed `de5b22b` reached the confirmation but a redundant exact-text status wait stopped before the selector. Remove that duplicate gate while retaining selector enforcement and add private-free status projection diagnostics. |
...
- Validation: direct fixtures cover a duplicate background reason with one modal reason, exact semantic preference, valid opaque-described right-footer selection, missing/duplicate context, missing/extra/both-empty/text-named footer nodes, normalized/containing counts, focused geometry, private-free empty-set flags, target-label absence, and exact-versus-containing status summaries. Focused 31/one intentional physical skip, CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. The clean pushed `de5b22b` run reached confirmation and proved exact status text is not independently exposed before the selector. Commit/push and corrected physical proof remain pending.
