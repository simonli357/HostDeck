...
| BUG-040 | Returning from direct not-found navigation can show cached ready UI while no server SSE subscriber exists. | High | Release blocker | In progress | `FE-V1-090` | The clean `c3fd9a4` run rendered `Ready to send` but retained zero active subscribers. The candidate reloads BFCache restoration and requires exactly one fresh detail read plus one fresh stream/subscriber before UI-ready truth. Unit 2,922/29 intentional skips, web 904, focused web/CLI 142/one intentional physical skip, root static, and planning gates pass. |
...
- Closed by: pending implementation commit and corrected physical evidence.
