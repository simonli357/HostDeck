...
| BUG-031 | Failure cleanup can leave a background Android Chrome process after returning Home. | High | Release blocker | In progress | `FE-V1-090` | Sixteen failed executions ended with Serve absent, Chrome stopped, dedicated profile selected, and clean worktree; the harness also proved launcher foreground before one unrelated app later interrupted the device. Successful-run absence proof remains. |
...
| BUG-036 | The physical recovery oracle searches for the top reconnect notice after prompt completion leaves that notice outside Android's visible hierarchy. | High | Release blocker | In progress | `FE-V1-090` | The clean `33cc627` run reached intentional stream recovery after all event and prompt checks but could not see the above-viewport notice. The candidate requires the exact fixed-composer write-block semantic in the current viewport, reports sanitized stream/request counters on failure, and passes 102 focused cases/one intentional physical skip plus CLI/web type/static/planning gates; corrected physical proof remains. |
