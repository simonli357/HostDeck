...
| BUG-043 | The physical 25-row Skills fixture violates strict deterministic name order and is rejected after one read. | High | Release blocker | In progress | `FE-V1-090` | The clean `6633c22` run and two bounded diagnostics proved a fully visible `/skills` row, one tap, one server `read_skills`, and the explicit sanitized failure surface. A direct fixture regression then reproduced `Skill snapshots must use strict deterministic name order`: `release-readiness` preceded later `physical-skill-*` rows. The candidate emits 24 ordered physical rows followed by `release-readiness` and asserts exact 25-row ascending order. Focused 35/one intentional physical skip, CLI typecheck, Biome, and planning pass; corrected physical proof remains. |
