...
| BUG-043 | Android omits exact fixed-footer Skills status text for the long physical fixture. | High | Release blocker | In progress | `FE-V1-090` | The clean `888fe19` run opened Skills after one full-row tap but did not expose exact footer child text `Skills capture current`. The candidate requires the content-only `Search skills` editor after the current read and `1 matching` after the bounded local filter; the terminal dashboard counter retains exact one-read proof. Focused 79/one intentional physical skip, CLI typecheck, Biome, and planning pass; corrected physical proof remains. |
