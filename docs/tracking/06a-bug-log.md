...
| BUG-030 | Approval confirmation action is not admitted by exact semantic, clickable, fully visible reveal on the real hierarchy. | High | Release blocker | In progress | `FE-V1-090` | Pushed `c8989d6` exposed one remaining text-only open check and pre-capture target retention; the candidate uses exact semantic detection and one fresh safe pre-tap acquisition. |
| BUG-031 | Failure cleanup can leave a background Android Chrome process after returning Home. | High | Release blocker | In progress | `FE-V1-090` | Twelve failed executions ended with Serve absent, launcher foreground, Chrome stopped, dedicated profile selected, and clean worktree. Successful-run absence proof remains. |
...
| BUG-033 | Approval sequencing can cross a stream reconnect and mistake prompt-composer geometry for the closed confirmation footer. | High | Release blocker | In progress | `FE-V1-090` | The candidate moves approval before event diagnostics, gates approval and prompt on stable write authority, and permits no anonymous action fallback; corrected physical proof remains. |
