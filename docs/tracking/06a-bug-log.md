...
| BUG-049 | Host evidence capture rejects the intentionally rendered private origin as if it were retained Chrome material. | High | Release blocker | In progress | `FE-V1-090` | The clean `5f5f4fd` run proved unique exact Host heading, structurally eligible back control and permission value, and the canonical inert origin; only the heading's irrelevant web-text structure failed. The candidate uses that heading solely as an exact intersecting identity landmark while retaining full checks on the action, permission, and redaction target. |
