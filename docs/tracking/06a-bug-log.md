...
| BUG-040 | Direct Android-intent navigation can confuse retained-tab authority with same-tab history recovery. | High | Release blocker | In progress | `FE-V1-090` | The clean `27d84ec` run proved exact external-tab closure and normal Chrome relaunch, then observed the intended `6fa0579` BFCache recovery: one fresh selected-detail read, one fresh stream/opened subscriber, and exactly one active subscriber. The corrected oracle now requires all four increments before UI-ready truth. Corrected phone proof remains pending. |
