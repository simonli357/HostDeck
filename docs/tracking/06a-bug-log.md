...
| BUG-039 | Full Chrome refresh retains deep Session Detail scroll while the stale-state oracle requires a top notice. | High | Release blocker | In progress | `FE-V1-090` | The stale detail request occurred, but `Showing stale session state` remained above the current hierarchy. Pushed `d0d31f7` reuses bounded dock-aware backward reveal before capture. CLI driver 32/one intentional physical skip, typecheck, and formatting pass. |
...
- Current evidence: the clean `b6c31f6` stale-oracle failure is the twentieth product-sequence post-mutation cleanup proof. One separate cellular preflight failure at `0fb3586` restored Wi-Fi, mobile data, Serve, profile, launcher, Chrome, tunnels, stay-awake, and worktree state before Chrome UI execution.
- Closed by: pending corrected physical evidence and implementation commit.
...
- Closed by: implementation `d0d31f7`; corrected physical evidence remains pending.
