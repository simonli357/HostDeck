...
| BUG-031 | Failure cleanup can leave a background Android Chrome process after returning Home. | High | Release blocker | In progress | `FE-V1-090` | Twenty-six product-sequence post-mutation failed executions ended with Serve absent, Chrome stopped, dedicated profile selected, and clean worktree; one cellular preflight failure also restored exact state. The harness also proved launcher foreground before one unrelated app later interrupted the device. Two separate auto-lock preflight refusals made no mutation. Successful-run absence proof remains. |
...
| BUG-041 | Physical `/plan` entry waits for a terminal status below the approved initial phone viewport. | High | Release blocker | In progress | `FE-V1-090` | The clean `5d2abfc` run proved the trigger disappeared after opening, so the sheet was not closed or safely reacquirable. Approved 390×844 evidence places `Plan control ready` below the fold while `Default`, `No pending change`, and `No observed Plan execution` own initial current truth. The candidate requires all three visible labels plus one exact Plan read; focused 105/one intentional physical skip and static/planning gates pass. |
