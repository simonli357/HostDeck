...
| BUG-030 | Approval confirmation action is not admitted by exact semantic, clickable, fully visible reveal on the real hierarchy. | High | Release blocker | In progress | `FE-V1-090` | Clean pushed `868b15a` proves the stable footer pattern is empty left plus opaque-described right. Require that pattern under exact context/geometry without reading the opaque value. |
...
- Candidate result: retain the explicit product label and prefer exact semantic selection. Only when semantics are absent, require unique exact title and reason context fully inside the Chrome page, then select the unique rightmost node from exactly two aligned bottom-row clickables with the observed empty-left/opaque-described-right pattern, bounded action size, lower-page position, symmetric margins, seam, and right-quadrant rules. Wait without swiping and tap once. No opaque-value read, text substring, arbitrary nearest-node, coordinate constant, retry, or raw hierarchy is used.
- Harsh success criteria: prefer one exact clickable semantic match fully inside the Chrome page. If the target phone exposes none, permit only the frozen confirmation-specific context, empty/set, and geometric contract above; reject missing/duplicate context, any extra/missing/both-empty/text-named footer node, invalid page/action/margin/alignment/seam/quadrant geometry, and every ambiguity. Perform one tap with no retry and retain exact pending-request and responding-state assertions.
...
- Fix: a dedicated confirmation action selector implements exact-first semantics and the frozen strict empty-left/opaque-described-right footer relationship. A bounded waiter records only private-free geometry/empty-set flags and never swipes the fixed sheet. The product keeps its explicit accessible label for browser/screen-reader semantics.
- Validation: direct fixtures cover exact semantic preference, valid opaque-described right-footer selection, missing/duplicate context, missing/extra/both-empty/text-named footer nodes, normalized/containing counts, focused geometry, private-free empty-set flags, and target-label absence. Focused 30/one intentional physical skip, CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. Corrected physical proof remains pending.
...
- Validation: direct regression freezes and asserts Home-before-force-stop ordering. Focused/static gates passed before the clean pushed runs; `1d37ef1`, `306859b`, `d57c523`, and `868b15a` each ended with Serve absent, launcher foreground, Chrome stopped, dedicated profile selected, and clean worktree. Successful-run physical absence proof remains pending.
- Closed by: pending corrected physical evidence and implementation commit.
