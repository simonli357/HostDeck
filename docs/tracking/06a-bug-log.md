...
| BUG-030 | Approval confirmation action is not admitted by exact semantic, clickable, fully visible reveal on the real hierarchy. | High | Release blocker | In progress | `FE-V1-090` | Clean pushed `d57c523` proves all expected/footer semantics and focus are absent while exactly two stable footer clickables remain. Use exact-first, strict context-bound anonymous right-footer geometry with fail-closed ambiguity and empty-set diagnostics. |
...
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: Xiaomi Chrome's UiAutomator projection omits both descendant-derived and explicit ARIA names for the confirmation footer. Clean pushed `d57c523` proves exact, whitespace-normalized, and containing matches for both public footer labels are zero, no node is focused, and only two stable clickables remain at the bottom row across four bounded observations. This is a platform projection limitation rather than a product DOM naming or scrolling defect; the next empty-set flags distinguish fully anonymous nodes from unrelated hidden names.
- Candidate result: retain the explicit product label and prefer exact semantic selection. Only when semantics are absent, require unique exact title and reason context fully inside the Chrome page, then select the unique rightmost node from exactly two aligned anonymous bottom-row clickables under bounded action size, lower-page position, symmetric margins, seam, and right-quadrant rules. Wait without swiping and tap once. No text substring, arbitrary nearest-node, coordinate constant, retry, or raw hierarchy is used.
- Harsh success criteria: prefer one exact clickable semantic match fully inside the Chrome page. If the target phone exposes none, permit only the frozen confirmation-specific geometric contract above; reject missing/duplicate context, any extra/missing/named footer node, invalid page/action/margin/alignment/seam/quadrant geometry, and every ambiguity. Perform one tap with no retry and retain exact pending-request and responding-state assertions.
...
- Fix: a dedicated confirmation action selector now implements exact-first semantics and the frozen strict footer relationship. A bounded waiter records only private-free geometry and never swipes the fixed sheet. The product keeps its explicit accessible label for browser/screen-reader semantics.
- Validation: direct fixtures cover exact semantic preference, valid anonymous footer selection, missing/duplicate context, missing/extra/named footer nodes, normalized/containing counts, focused geometry, private-free text/description empty-set flags, and target-label absence. Focused 30/one intentional physical skip, CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. Corrected physical proof remains pending.
...
- Validation: direct regression freezes and asserts Home-before-force-stop ordering. Focused/static gates passed before both clean pushed failed runs; `1d37ef1` and `306859b` each ended with Serve absent, launcher foreground, Chrome stopped, dedicated profile selected, and clean worktree. Successful-run physical absence proof remains pending.
- Closed by: pending corrected physical evidence and implementation commit.
