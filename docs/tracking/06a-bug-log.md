...
| BUG-040 | Direct Android-intent navigation can confuse retained-tab authority with same-tab history recovery. | High | Release blocker | In progress | `FE-V1-090` | Pushed `6fa0579` closes the real BFCache lifecycle gap. The clean `a59d0bc` run proved one missing read with unchanged selected-session authority, then confirmed Android Back minimizes Chrome, closes the external tab, and drops the background stream rather than foregrounding the retained tab. The current candidate launches Chrome normally after exact zero-subscriber proof and requires one exact stream reopening with no detail/read duplication. Corrected phone proof remains pending. |
...
- Harsh success criteria: retain the tested pagehide/BFCache lifecycle binding. Begin direct not-found proof with exactly one active selected-session subscriber and snapshot selected-detail reads, missing-detail reads, stream requests, opened subscribers, and active subscribers. Opening the missing-session intent must add exactly one missing-detail read and change no selected-session counter or subscriber. One Android Back action must close only the active stream while changing no request/open count; one normal Chrome launcher action must return to exact `Ready to send` truth with exactly one new stream request/opened subscriber, one active subscriber, no selected-detail reread, retry, or mutation. Reject every missing, lost, duplicate, or extra count; retain private-free diagnostics and exact cleanup.
- Fix: `6fa0579` adds the tested browser lifecycle owner. The corrected physical candidate separately counts missing-detail reads, freezes all selected-session authority counters, removes the second direct selected-session intent, proves exact background stream closure, and relaunches Chrome through its normal launcher alias before requiring one exact stream reopening.
- Validation: lifecycle coverage remains unit 2,922/29 intentional skips, web 904, adjacent web/CLI 142/one intentional physical skip, root static, and planning gates. The first corrected driver passed direct 34/one intentional physical skip, CLI typecheck, and Biome, and the clean `a59d0bc` phone run passed missing-tab authority before exposing the Back/minimize behavior. Updated direct/static and corrected no-retry phone proof remain pending.
- Closed by: product lifecycle implementation `6fa0579`; physical navigation correction and aggregate evidence remain pending.
