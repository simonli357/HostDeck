...
| BUG-048 | The physical Session Detail Host step still expects a direct trigger removed by the approved Session Actions consolidation. | High | Release blocker | In progress | `FE-V1-090` | The clean `8cd62f5` run reached Host access after exact Interrupt completion but could not find a direct trigger; integrated UI tests require Host as the fourth Session Actions row. The candidate opens and measures the exact clickable row, requires the embedded Host page plus back affordance, and closes the owning Session Actions dialog. Corrected physical proof remains. |
