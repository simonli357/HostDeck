...
| BUG-031 | Failure cleanup can leave a background Android Chrome process after returning Home. | High | Release blocker | In progress | `FE-V1-090` | Fourteen failed executions ended with Serve absent, launcher foreground, Chrome stopped, dedicated profile selected, and clean worktree. Successful-run absence proof remains. |
...
| BUG-035 | After approval moved before event diagnostics, the first replay-boundary row is above the viewport but its reveal path swipes only toward later activity. | High | Release blocker | In progress | `FE-V1-090` | The clean `e59e1c4` run refused two anonymous details actions after four wrong-direction swipes. The candidate searches backward only for the first boundary, forward for later ordered events, retains exact label-to-preceding-action ownership, permits at most four content-only swipes, and passes 31 direct harness cases/one intentional physical skip plus CLI type/static/planning gates; corrected physical proof remains. |
