...
| BUG-045 | Clipboard cleanup opens a second external Session Detail tab while the retained selected-session tab remains live. | High | Release blocker | In progress | `FE-V1-090` | The clean `633a7c7` run copied the Resume command, cleared the clipboard, then could not settle at one subscriber because direct intent navigation created another selected-session tab. The candidate closes the clipboard tab with Android Back, relaunches the retained Chrome tab normally, and requires exact before/backgrounded/returned detail and stream counters plus private-free diagnostics. Corrected physical proof remains. |
