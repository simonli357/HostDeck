...
| BUG-031 | Failure cleanup can leave a background Android Chrome process after returning Home. | High | Release blocker | In progress | `FE-V1-090` | Seventeen post-mutation failed executions ended with Serve absent, Chrome stopped, dedicated profile selected, and clean worktree; the harness also proved launcher foreground before one unrelated app later interrupted the device. One separate auto-lock preflight refusal made no mutation. Successful-run absence proof remains. |
...
| BUG-036 | The physical recovery fixture injects a terminal storage error while requiring the browser to reconnect automatically. | High | Release blocker | In progress | `FE-V1-090` | The clean pushed `bb6d571` run reached recovery and reported one `500`, zero active subscribers, and no further request because production correctly treats storage failure as non-retryable. The candidate removes that invalid injection, holds exactly one reconnect request after `source_failed`, requires and captures the visible composer write block, releases the same request to one new subscriber, rejects duplicate gate use, and adds sanitized gate/stream diagnostics. Focused 184 cases/one intentional physical skip plus CLI type/static checks pass; corrected physical proof remains. |
