...
| BUG-049 | Host evidence capture rejects the intentionally rendered private origin as if it were retained Chrome material. | High | Release blocker | In progress | `FE-V1-090` | The clean `2b9680a` run reached Host access and passed its live check, then failed before writing `fe090-32-host-access.png`. The candidate admits only exact inert full-origin text under an exact product label, redacts its bounded pixels before writing, and keeps every browser, hostname-only, clickable, descriptive, unknown-private, and malformed case fail-closed. |
