...
| BUG-050 | The post-revocation Host-lock step uses one stale confirmation-entry tap and later identifies the destructive action by duplicated text alone. | High | Release blocker | In progress | `FE-V1-090` | The clean `ac9b5d5` run acquired one exact clickable `Lock writes` target after revocation, but its one tap opened no confirmation and made no lock transition. The candidate permits one non-mutating retry only after strict visible/clickable reacquisition and zero lock audit, then selects the destructive action only from exact lock-dialog geometry and taps it once. |
...
- Current evidence: the clean `c3fd9a4` direct-return stream failure is the twenty-first product-sequence post-mutation cleanup proof. One separate cellular preflight failure at `0fb3586` restored Wi-Fi, mobile data, Serve, profile, launcher, Chrome, tunnels, stay-awake, and worktree state before Chrome UI execution.
- Closed by: pending corrected physical evidence and implementation commit.
...
### BUG-050 Post-Revoke Host Lock Entry Is Stale

- Symptom: the clean `ac9b5d5` run crossed the redacted Host capture, Office-device confirmation, exact one-device revocation, and terminal capture, then acquired and measured one exact clickable `Lock writes` node. One tap left the same target and Host state visible and never opened `Lock remote writes?`.
