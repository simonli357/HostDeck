...
| BUG-030 | Approval confirmation action is not admitted by exact semantic, clickable, fully visible reveal on the real hierarchy. | High | Release blocker | In progress | `FE-V1-090` | The selector now rejects the disproven anonymous-footer fallback and requires one exact semantic action inside exact modal context; corrected physical proof remains. |
| BUG-031 | Failure cleanup can leave a background Android Chrome process after returning Home. | High | Release blocker | In progress | `FE-V1-090` | Eleven failed executions ended with Serve absent, launcher foreground, Chrome stopped, dedicated profile selected, and clean worktree. Successful-run absence proof remains. |
...
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: the original exact-text failure was misclassified as an Android naming gap. Manual inspection of a privacy-cropped `d11619a` failure capture proves that title, status, close control, and risk marker were absent because the approval sheet had correctly closed during authority loss; the surviving wide-plus-narrow controls were the disabled prompt composer. The anonymous relational fallback could therefore admit the wrong control family.
...
- Fix: the dedicated selector keeps unique title/status plus one geometrically modal reason and now returns only the exact semantic action. The disproven anonymous empty/set footer and geometry fallback is removed. A bounded waiter records only private-free context counts/geometry and never swipes the fixed sheet. The product keeps its explicit accessible label for browser/screen-reader semantics.
- Validation: direct fixtures cover exact semantic success; anonymous, duplicate, missing, containing-only, non-clickable, and out-of-page rejection; relational modal context; and private-free context summaries. Component regression requires `{ preventScroll: true }` while preserving Cancel focus and origin restoration. Focused component/driver 40/one intentional physical skip, CLI typecheck, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. Lint, commit/push, and corrected physical proof remain pending.
...
- Validation: direct regression freezes and asserts Home-before-force-stop ordering. Focused/static gates passed before the clean pushed runs; `1d37ef1`, `306859b`, `d57c523`, `868b15a`, `0e1781a`, `43b27a5`, `de5b22b`, `6a1c8c9`, and three separate `d11619a` executions each ended with Serve absent, launcher foreground, Chrome stopped, dedicated profile selected, and clean worktree. Successful-run physical absence proof remains pending.
- Closed by: pending corrected physical evidence and implementation commit.
...
- Validation: direct boundary fixtures and focused component/driver 40/one intentional physical skip, CLI typecheck, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. Lint, commit/push, and corrected physical proof remain pending.
- Closed by: pending corrected physical evidence and implementation commit.
