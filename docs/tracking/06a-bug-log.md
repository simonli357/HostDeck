...
| BUG-024 | Physical Session Detail does not retain its required live SSE subscriber after successful one-tap navigation and detail read. | High | Release blocker | In progress | `FE-V1-090` | The clean pushed `958f933` run proved native clickability, one-tap navigation, one detail GET, and rendered detail truth while the active subscriber stayed at zero. Stream request/status and subscriber lifecycle counters now isolate the remaining browser/server boundary. |
...
- Symptom: after the earlier ambiguous timeouts, the clean pushed `958f933` run reported `detail_requests=1`, `active_subscribers=0`, a clickable fully visible trigger, the corresponding detail-route semantic node, and rendered `Back to Mission Control` plus retained-boundary truth.
...
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Ruled out: target naming, 44 CSS-pixel geometry, Chrome page containment, native clickability, tap dispatch, client navigation, detail routing, paired read authority, and the detail GET are proven. The visibility guard remains valid hardening but was not the timeout root cause.
- Diagnostic fix: preserve the existing native/route diagnosis and add only private-free stream request/status, subscriber open/active/abort/explicit-close/source-failure, and approved visible stream-state fields. This distinguishes no browser request, HTTP rejection, open failure, and immediate lifecycle closure without retaining origin, authority, content, or device identity.
- Validation: the realistic Android hierarchy regression and bounded summary checks pass in the 27-test focused driver suite with one intentional physical skip; CLI typecheck passes. The prior full 2,912-unit and unchanged deterministic package gates remain valid because this candidate changes diagnostics only. One no-retry diagnostic physical run remains pending.
