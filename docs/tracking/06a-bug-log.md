...
| BUG-024 | The physical dashboard replay fixture violates the production boundary-handoff contract, so Session Detail receives a fail-closed `500` instead of a live SSE subscriber. | High | Release blocker | In progress | `FE-V1-090` | The clean pushed `74a6203` run isolated one stream request, one `500`, one source-open failure, and zero opened subscribers. Deep-frozen fixture events plus truthful boundary truncation pass a direct five-event replay-to-live regression; corrected physical acceptance remains pending. |
...
| BUG-026 | The physical dashboard requires a below-fold replay-boundary label in the current Android hierarchy before using its existing bounded reveal owner. | High | Release blocker | In progress | `FE-V1-090` | The clean pushed `1ccf27e` run established live writable Session Detail, then failed only at the unscrolled boundary-label wait. Replay drain plus visible-current evidence now gates the top capture; the event diagnostic retains bounded row reveal and inspection. |
...
### BUG-026 Initial Detail Gate Assumes Below-Fold Boundary Visibility

- Symptom: the clean pushed `1ccf27e` run established one live Session Detail subscriber and rendered the writable composer, then timed out waiting for `Earlier activity unavailable` in Android's current unscrolled hierarchy.
...
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: `waitForAndroidUiText` observes only the current Chrome accessibility hierarchy and never scrolls. Once the complete replay populates the mobile timeline, its first boundary row may be below the initial viewport. The immediately following `runPhysicalEventDiagnostic` already owns bounded forward reveal for that exact row, making the earlier visibility assertion both stronger than the UI contract and redundant.
- Fix: before the initial top-of-detail capture, require one active subscriber, zero remaining server replay events, and visible `Current` replay-to-live truth. Keep the existing bounded row reveal, exact event read, limitation check, and screenshot as the boundary's physical evidence owner.
- Validation: direct five-event boundary replay and the prior physical live/writable transition prove event production and stream ownership. Focused/static checks and corrected no-retry physical acceptance remain pending.
- Closed by: pending corrected physical evidence and implementation commit.
