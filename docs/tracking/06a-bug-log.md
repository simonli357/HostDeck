...
| BUG-026 | The physical detail sequence assumes top context and below-fold replay truth are already present in the current Android hierarchy. | High | Release blocker | In progress | `FE-V1-090` | Clean pushed runs through `a3ad76f` establish live writable detail and replay drain, then isolate retained vertical offset. A bounded backward reveal now owns top `Current` evidence; the event diagnostic retains bounded forward row reveal and inspection. |
...
- Validation: direct five-event boundary replay and clean pushed physical live/writable/replay-drain transitions prove event production and stream ownership. Focused/static checks and corrected no-retry physical acceptance remain pending.
- Closed by: pending corrected physical evidence and implementation commit.
