...
| BUG-041 | Physical `/plan` acceptance waits for generic lifecycle status below the approved phone viewport. | High | Release blocker | In progress | `FE-V1-090` | The clean `8574fb7` run proved initial entry with one exact read, one Plan selection, and one pending write, then failed because `Saving next-turn mode` is below the 390×844 viewport. Approved current/submitting/staged targets expose exact mode/lifecycle truth above the fold. The clean `8a8b384` run then proved a CSS-transformed rail caption is absent from Android's hierarchy and failed closed after opening, before selection. The corrected candidate discriminates state-rail mode from the option through hierarchy-proven label multiplicity and verifies unique submitting/staged text without scrolling or resending. Corrected physical proof remains. |
