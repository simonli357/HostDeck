...
| BUG-049 | Host evidence capture rejects the intentionally rendered private origin as if it were retained Chrome material. | High | Release blocker | In progress | `FE-V1-090` | The clean `4c5dbc5` run again reached Host after its live check; the explicit redaction path rejected the real node before label matching and wrote no image. A bounded diagnostic candidate reports only origin/hostname relation flags, registered-value flags, allowlisted class category, interaction/ownership flags, bounds, and exact known-label counts so the next run can distinguish product text from Chrome/private leakage without retaining a value. |
