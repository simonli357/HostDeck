...
| BUG-037 | Accepted prompt progress masks a later composer availability block, leaving disabled writes without a visible cause. | High | Release blocker | In progress | `FE-V1-090` | The clean pushed `0fb3586` run proved the reconnect state and blocked writes, but the composer retained `Turn completed` instead of exposing the reconnect reason. The candidate prioritizes non-turn availability failures, preserves accepted-turn progress semantics, and restores accepted status after recovery. Web 902, focused server/CLI/web 187 with one intentional physical skip, CLI/web typecheck, and formatting pass; corrected physical proof remains. |
