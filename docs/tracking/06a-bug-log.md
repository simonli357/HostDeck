...
| BUG-027 | A physical event diagnostic can treat an Android hierarchy action beneath the fixed session-control dock as tappable. | High | Release blocker | In progress | `FE-V1-090` | `5d8b014` rejected the occluded action but did not admit it after bounded reveal. A private-free per-swipe geometry summary now isolates the remaining real-hierarchy mismatch without logging raw UI or product content. |
...
- Validation: direct safe/top-occluded/bottom-occluded/incomplete-header/incomplete-dock/ambiguous-label/distant-action and swipe-lane geometry checks pass in the focused suite: 29 passed with one intentional physical skip. CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. The clean pushed `5d8b014` run failed closed at the third row and restored Serve, Chrome, tunnels, settings, and the clean worktree exactly; corrected no-retry physical proof remains pending.
