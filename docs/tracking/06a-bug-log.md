...
| BUG-043 | Physical Skills acceptance conflates one-read page entry with immediate first-hierarchy search availability. | High | Release blocker | In progress | `FE-V1-090` | `3e6be9e` fixes and directly regresses the invalid 25-row ordering, but its clean phone run reached the same opaque search assertion. The candidate first requires exact `/skills` title and one server read, then bounded-forward-reveals the labelled editor while recording private-free phase/editor geometry; terminal counters require one Skills read. Focused 35/one intentional physical skip, CLI typecheck, Biome, and planning pass; corrected physical proof remains. |
