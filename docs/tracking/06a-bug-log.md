...
| BUG-027 | Physical event interaction geometry conflates Chrome screenshot-toolbar bounds with app-overlay-safe tap bounds. | High | Release blocker | In progress | `FE-V1-090` | `351891b` proves one swipe moves the pair into the app-safe band, then Chrome toolbar collapse alone invalidates the screenshot selector. Interaction admission must use stable compositor plus app-overlay geometry. |
...
- Blocks: corrected no-retry physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: the original diagnostic accepted a hierarchy-present action beneath the fixed dock. The first correction measured app overlays but built their content region through `selectChromePageViewport`, whose deliberately strict screenshot contract requires an expanded, geometrically valid Chrome toolbar. Chrome may collapse that toolbar during page scroll even though the compositor and app overlay nodes remain valid for interaction.
- Harsh success criteria: acquire each label and nearest action as one pair; require a unique sticky back control plus unique measured `/model`, `/goal`, `/plan`, and utility dock controls; derive interaction bounds from one stable Chrome compositor and those app overlays without requiring expanded browser-toolbar geometry; require both pair members inside the resulting region with a 24-device-pixel overlay inset; scroll forward no more than four times from that same lane; keep screenshot cropping on its stricter toolbar-aware contract; fail closed on absent or ambiguous geometry; issue one tap only after admission; and retain the exact one-read plus expected-heading assertion.
...
- Fix: interaction geometry now requires one bounded Chrome compositor and derives its inset content/swipe region from that compositor, the sticky app back control, and all four fixed-dock controls. It no longer requires expanded browser-toolbar geometry. Screenshot selection remains unchanged and strict; the pair/app-overlay rules and private-free timeout summary remain intact.
- Validation: direct safe/top-occluded/bottom-occluded/incomplete-header/incomplete-dock/ambiguous-label/distant-action and swipe-lane geometry checks pass in the focused suite: 29 passed with one intentional physical skip. A collapsed-toolbar regression proves strict screenshot viewport selection fails while compositor/app-overlay interaction still admits the exact pair and retains content/swipe geometry. CLI typecheck, lint across 829 files, package exports, and the 220-task/84-requirement/683-dependency planning graph pass. Corrected no-retry physical proof remains pending.
