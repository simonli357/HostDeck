...
- Blocks: corrected no-retry aggregate physical acceptance, retained phone deployment, and all downstream release tasks.
- Root cause: unresolved at the Android input boundary. The stale-controller hypothesis is disproven because strict reacquisition returned the same enabled target before the second tap. A focused current-Chromium regression performs the same non-self revocation, proves `elementFromPoint` resolves to the lock button or its child at the exact center, and opens confirmation with a real touchscreen tap. Static sheet stacking, the retained revocation dialog, and shared controller sequencing therefore do not reproduce outside the Xiaomi/ADB path. The destructive-dialog selector remains required independently because the origin and confirmation can both expose `Lock writes` after entry succeeds.
...
- Current implementation: Android hierarchy parsing retains explicit disabled state, all clickable reveal/tap paths reject disabled nodes, and verified non-mutating entry accepts a caller-owned strict reacquisition selector. Host-lock entry requires one fully visible enabled action and zero lock audit, then allows at most one exact strict reacquisition before failing. The destructive action has a dedicated selector requiring one exact title, one enabled visible Cancel action, and one enabled visible `Lock writes` action below the title in Cancel's bounded footer row; the underlying origin action is rejected. One accepted plus one terminal lock audit is required after the single destructive tap. This candidate is not sufficient on the phone and is not a completed fix.
- Validation: the direct dialog matrix selects the owned confirmation action despite an underlying decoy and rejects missing/duplicate title, missing Cancel, disabled/non-clickable confirmation, wrong footer row, duplicate confirmation, and out-of-page geometry. Android parsing and generic reveal coverage reject explicit `enabled=false`. Focused Android/Host-lock/app-shell/device tests pass 63 with one intentional physical skip; CLI typecheck and Biome pass. The new current-Chromium touchscreen regression passes after the same non-self revocation and confirms a topmost center hit with zero mutation. The clean `cf3acba` phone run failed after both bounded entry taps and restored empty Serve, stopped Chrome, original phone settings, zero tunnels, and clean acceptance state. Private-safe Android diagnosis and corrected no-retry phone proof remain pending.
