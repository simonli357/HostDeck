...
| --- | --- | --- | --- | --- |
| 1 | `FE-V1-090` Harden the complete mobile dashboard on the real phone | in_progress | none | Prove sustained healthy cellular Tailscale product-path and ADB reachability, then run one fresh clean committed candidate containing pushed `2017215` with the complete BUG-056/BUG-060-to-BUG-066 correction through pairing, every dashboard system, exact runtime recovery, the real TalkBack transcript in `BUG-052`, generation-bound strict archive crossing in `BUG-051`, self-revocation, final aggregate, exact cleanup, private evidence review, and retained deployment. |
