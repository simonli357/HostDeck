...
| 4 | `FND-V1-103` Shared exact-data helpers | ready | none | 98 non-test deep-freeze copies across 7 behaviour classes; fold into the existing contracts exact-data module. |
| 5 | `IFC-V1-112` Resource observations | ready | none | The plan gives all 99 resource limits a `hostdeck.resource.<key>` observation name that nothing emits; either wire counters onto the diagnostics surface or record the deferral. |
...
Review remediation from `artifacts/rev-2026-08-21-architecture-review.md` is queued as `FND-V1-103`; `IFC-V1-111` closed the diagnostics gap; `FND-V1-106` closed `BUG-092`; `INT-V1-110` closed under `DEC-031`; `FND-V1-107` closed the nondeterministic unit gate, with `FND-V1-104`, `FND-V1-105`, and `IFC-V1-112` ordered behind them. `BUG-090` and `BUG-091` are closed in place.
