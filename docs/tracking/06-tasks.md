...
| --- | --- | --- | --- | --- |
| 1 | `FE-V1-100` Independent pre-phone physical-driver review | ready | none | Review the corrected batch, aggregate call graph, hostile tests, and nonphysical evidence before any phone execution. |

`FE-V1-092` through `FE-V1-099` are implemented and pushed as the corrected pre-phone batch. `FE-V1-100` is ready for independent review and remains open; the implementation agent must not close it. `FE-V1-090`, the phone, Tailscale, Serve state, package repinning, and failed candidates remain out of scope until that review passes.
