...
| 4 | `IFC-V1-112` Resource observations | ready | none | The plan gives all 99 resource limits a `hostdeck.resource.<key>` observation name that nothing emits; either wire counters onto the diagnostics surface or record the deferral. |
| 5 | `FND-V1-104` Operation descriptor design | ready | none | Nine Codex operations repeat one shape across ~45 files and ~30k lines; design the descriptor and decompose the sweep into per-operation leaf tasks. |
