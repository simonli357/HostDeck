...
| 4 | `IFC-V1-111` Runtime diagnostics channel | ready | none | An unexpected production failure currently leaves only a counter; recover forensics within the existing privacy and residue rules. |
| 5 | `INT-V1-110` Codex acceptance-band spike | ready | none | Exact 0.144.0 equality means a shipped package dies on the next Codex release; measure whether the existing capability rules can carry a band. |
| 6 | `FND-V1-103` Shared exact-data helpers | ready | none | 98 non-test deep-freeze copies across 7 behaviour classes; fold into the existing contracts exact-data module. |
| 7 | `FND-V1-106` Envelope produce/parse split | ready | none | The sensitive-detail-key guard cannot be widened until creating and parsing an envelope validate separately. |
...
Review remediation from `artifacts/rev-2026-08-21-architecture-review.md` is queued as `IFC-V1-111`, `INT-V1-110`, `FND-V1-103`, and `FND-V1-106`; `FND-V1-107` closed the nondeterministic unit gate, with `FND-V1-104`, `FND-V1-105`, and `IFC-V1-112` ordered behind them. `BUG-090` and `BUG-091` are closed in place.
