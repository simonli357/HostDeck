...
| --- | --- | --- | --- | --- |
| 1 | `IFC-V1-028` Implement pairing-code creation and remote claim | in_progress | none | Composes completed trust, pairing/rate storage, route manifest, and security audit execution. |
| 2 | `IFC-V1-030` Implement host lock state and lock/unlock boundary | ready | none | Consumes the completed authentication, CSRF, settings, manifest, and security-audit boundaries. |
| 3 | `IFC-V1-031` Implement LAN configuration and enable/disable boundary | ready | none | Consumes the completed HTTPS enrollment, trust, CSRF, manifest, and security-audit boundaries. |
