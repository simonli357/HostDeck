...
- Active task: `INT-V1-020` passive/agentic goal lifecycle, second of the human-designated `/model`, `/goal`, `/plan` controls.
- Other ready work includes `INT-V1-021` to `INT-V1-025`; `INT-V1-018` is dependency-gated until Plan composition. Independent ready work includes `DAT-V1-022` projection retention, `DAT-V1-023` audit state, `IFC-V1-018` fanout, `IFC-V1-019` route manifest, `IFC-V1-047` HTTP limits, and `IFC-V1-015` physical-device HTTPS.
...
- `IFC-V1-020` defines one strict resource policy across HTTP, SSE, admission, protocol, lifecycle, and CLI boundaries; `INT-V1-017` and `INT-V1-019` extend it to 64 fields with pending-notification, model-catalog, and pending-model ceilings. Stable oversized/overload codes, exact Fastify/Codex mappings, and monotonic deadline forms pass. Evidence: `artifacts/ifc-v1-020-resource-budget-deadline.md`, `artifacts/int-v1-017-codex-event-normalization.md`, `artifacts/int-v1-019-pending-model-control.md`.
...
- Remaining production Codex controls, combined prompt/model/Plan dispatch, approval routing, restart behavior, and live HostDeck composition; exact external semantics, ordered normalization/projection, and standalone pending model control are proven as ports, but the connection callback is not assembled.
...
- Next action: implement `INT-V1-020` exact paused versus agentic goal lifecycle with the unapplied next-turn setting guard.
