...
- Active task: `INT-V1-026` exact active-turn interrupt without archive semantics.
- Other ready runtime work includes `INT-V1-022` to `INT-V1-024`. Independent ready work includes `DAT-V1-023` audit state, `IFC-V1-018` fanout, `IFC-V1-019` route manifest, `IFC-V1-047` HTTP limits, and `IFC-V1-015` physical-device HTTPS.
...
- `IFC-V1-020` defines one strict resource policy across HTTP, SSE, admission, protocol, lifecycle, and CLI boundaries; `INT-V1-017`, `INT-V1-019` to `INT-V1-021`, `DAT-V1-022`, and `INT-V1-025` extend it to 69 fields with pending-notification, control-state, projection-retention, and approval-expiry ceilings. Stable oversized/overload codes, exact Fastify/Codex mappings, and monotonic deadline forms pass. Evidence: `artifacts/ifc-v1-020-resource-budget-deadline.md`, `artifacts/int-v1-017-codex-event-normalization.md`, `artifacts/int-v1-019-pending-model-control.md`, `artifacts/int-v1-020-goal-control.md`, `artifacts/int-v1-021-plan-control.md`, `artifacts/dat-v1-022-production-projection-retention.md`, `artifacts/int-v1-025-approval-routing.md`.
...
- Remaining production Codex usage/compact/skills utilities, interrupt routing, restart behavior, and live HostDeck composition; exact external semantics, ordered normalization/projection, prompt/model/goal/Plan/approval controls, and their real boundaries are proven as ports, but the connection callbacks are not assembled.
...
- Next action: implement `INT-V1-026` confirmed interrupt for one exact active turn.
