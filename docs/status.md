...
- Active task: `INT-V1-019` pending next-turn model selection, first of the human-designated `/model`, `/goal`, `/plan` controls.
- Other ready work includes `INT-V1-018`, `INT-V1-020` to `INT-V1-025`, `DAT-V1-022` projection retention, `DAT-V1-023` audit state, `IFC-V1-018` fanout, `IFC-V1-019` route manifest, `IFC-V1-047` HTTP limits, and `IFC-V1-015` physical-device HTTPS.
...
- `IFC-V1-020` defines one strict resource policy across HTTP, SSE, admission, protocol, lifecycle, and CLI boundaries; `INT-V1-017` extends it to 60 fields with the Codex pending-notification ceiling. Stable oversized/overload codes, exact Fastify/Codex mappings, and monotonic deadline forms pass. Evidence: `artifacts/ifc-v1-020-resource-budget-deadline.md`, `artifacts/int-v1-017-codex-event-normalization.md`.
...
- Production Codex controls, approval routing, restart behavior, and live HostDeck composition; exact external semantics and the ordered normalization/projection pipeline are proven as ports, but the connection callback is not assembled.
...
- Next action: implement `INT-V1-019` exact model catalog/current/pending selection and turn-start dispatch semantics.
