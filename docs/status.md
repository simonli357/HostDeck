...
- Active task: `INT-V1-113` loaded-thread reconciliation, automatic enrollment, and bounded live notification handling.
...
- Validation: shared broker ownership/security/concurrency tests and a real no-model exact-Codex standard-socket lifecycle pass alongside exact binding, native UUID state, full unit/contract suites, typecheck, lint, planning, binding, and selected-runtime boundary. Automatic runtime enrollment/API/UI implementation remains.
- Git: implementation is isolated and pushed through INT-V1-112 on `feat/shared-codex-runtime`; the primary dirty worktree remains untouched.
