...
- Active task: `FE-V1-108` is blocked only because the Android phone is currently absent from ADB and the laptop Tailscale peer set. Non-device `REL-V1-110` preparation may continue meanwhile.
...
- Next chain: connect/unlock Android -> install the candidate -> `FE-V1-108` -> `REL-V1-110`.
