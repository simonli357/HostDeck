...
- Active task: `INT-V1-112` standard-socket broker lifecycle and independent HostDeck attachment.
...
- Validation: exact binding regeneration, real Unix IPC, native UUID read/resume, automatic-enrollment migration/rollback/restart/two-handle evidence, 307 contract tests, 3,244 unit tests, typecheck, lint, planning, binding, and selected-runtime boundary pass. Broker/API/UI implementation remains.
- Git: implementation is isolated and pushed through DAT-V1-106 on `feat/shared-codex-runtime`; the primary dirty worktree remains untouched.
