...
- Active task: `INT-V1-021` pending Plan/Default selection, third of the human-designated `/model`, `/goal`, `/plan` controls.
- Other ready runtime work includes `INT-V1-022` to `INT-V1-025`; `INT-V1-018` is dependency-gated until Plan composition. Independent ready work includes `DAT-V1-022` projection retention, `DAT-V1-023` audit state, `IFC-V1-018` fanout, `IFC-V1-019` route manifest, `IFC-V1-047` HTTP limits, and `IFC-V1-015` physical-device HTTPS.
...
- `IFC-V1-020` defines one strict resource policy across HTTP, SSE, admission, protocol, lifecycle, and CLI boundaries; `INT-V1-017`, `INT-V1-019`, and `INT-V1-020` extend it to 65 fields with pending-notification, model-catalog, pending-model, and uncertain-goal ceilings. Stable oversized/overload codes, exact Fastify/Codex mappings, and monotonic deadline forms pass. Evidence: `artifacts/ifc-v1-020-resource-budget-deadline.md`, `artifacts/int-v1-017-codex-event-normalization.md`, `artifacts/int-v1-019-pending-model-control.md`, `artifacts/int-v1-020-goal-control.md`.
...
- Remaining production Codex controls, combined prompt/model/Plan dispatch, approval routing, restart behavior, and live HostDeck composition; exact external semantics, ordered normalization/projection, pending model, and passive/agentic goal control are proven as ports, but the connection callback is not assembled.
- Production composition with selected Codex pipeline/trust/route registrations, replay/live handoff and subscriber queues, full raw-socket Node-limit enforcement, continuous fanout, mutable runtime health, retention invocation, or complete application drain; the generic listener lifecycle and individual adapters are proven, not those downstream integrations.
...
- Next action: implement `INT-V1-021` exact Plan/Default catalog, pending revision, model composition, and settings/plan-event confirmation.
