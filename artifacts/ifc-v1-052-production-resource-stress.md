...
- Implementation: `tests/production-resource-stress.integration.test.ts`; criteria `19aa0de`; aggregate `59fc5e8`; explicit stream-failure observation `4986498`.
- Six real-listener scenarios pass. They prove the exact 22-registration/35-route graph and reviewed budget, occupied-port construction cleanup and same-port restart, exact 4 KiB selected body acceptance plus one-byte-over rejection, partial-upload isolation, two repeated synchronized contention cycles, no-send/possible-send response-loss truth, and active SSE/upload/idle/read/mutation shutdown with immediate restart.
- The repeated cycles reach exactly four HTTP owners and two broker owners. They record two each of HTTP, CLI, per-actor, exact-target, global-mutation, subscriber-admission, and queue-overflow rejections; two usage and two compact protocol frames; peak two mutation owners/targets; peak eight retained SSE events; four successful mutation settlements; eight selected audit rows; and zero current HTTP, SSE, admission, or broker owners after each cycle.
- Failure truth stays explicit: a no-send compact records terminal failed timeout with no frame; response loss records one immutable accepted/incomplete audit pair and one frame; a late response is retired; matching projection events alone reconcile compact state. An aborted partial upload produces one bounded redacted `ECONNRESET` observer record, and the two intentional stalled streams produce only `{code: queue_overflow, cursor: 9}` observations.
- Focused aggregate: 6/6. Adjacent HTTP/SSE/admission/deadline/CLI owners: 66/66. Full workspace: unit 1,773 passed with 26 intentional external skips, contract 238/238, integration 25/25 across 16 files, and web 20/20.
- Root and all eight package typechecks, lint/exports over 506 files and eight packages, scaffold over eight packages/20 scripts, planning at 212 tasks/84 requirements/649 dependencies, and the 602-production-module/21-external-module runtime boundary pass. The exact isolated Codex 0.144.0 binding verifies 671 files at `e1a1a5cff3ab91862f9215dd06538eae1ea0b00bae48cbb7d87061faaee27e24`; the default 0.144.5 binary correctly remains ineligible.
- Frozen offline install is unchanged. Production audit reports no known vulnerabilities. All 137 production license entries across 140 installed paths use the seven reviewed permissive expressions. Staged diff, private-data/no-retry/source-boundary inspection, active socket/timer/listener comparison, and final `hostdeck-resource-stress-*` root inspection pass with zero task residue.
- No production source, route, schema, authority, dependency, lockfile, setup, Tailscale state, Serve state, browser, or phone state changed. Compiled output, packaged `codexdeck`, dashboard assets, user services, clean-machine parity, interface-module hardening, and release readiness remain owned by `IFC-V1-021`, `IFC-V1-053` to `IFC-V1-058`, `IFC-V1-091`, frontend, and release tasks.
