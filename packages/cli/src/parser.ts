...
export function parseCliArgs(args: readonly string[]): ParsedCliArgs {
  ...
  if (command === "broker") {
    return { command: parseBrokerCommand(rest, json), configFlags };
  }
...
function parseBrokerCommand(
  args: readonly string[],
  json: boolean
): Extract<ParsedCliCommand, { readonly kind: "broker" }> {
  if (
    args.length !== 1 ||
    (args[0] !== "start" && args[0] !== "status" && args[0] !== "stop")
  ) {
    throw usageFailure("The broker command requires exactly one action: start, status, or stop.");
  }
  return { kind: "broker", action: args[0], json };
}
