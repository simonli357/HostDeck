...
const pairedWriterId = "client_approval_route_writer";
const pairedWriterToken = "W".repeat(43);
const pairedWriterCsrf = "C".repeat(43);
const pairedReaderId = "client_approval_route_reader";
...
describe("selected approval routes", () => {
  ...
  it("preserves read parity over paired HTTPS and admits only a CSRF-authorized writer response", async () => {
    const harness = await createPairedHarness();
    try {
      const writerRead = await secureApproval(harness, "writer", "GET");
      expect(writerRead.statusCode, writerRead.body).toBe(200);
      expect(writerRead.json()).toEqual({ target: managedTarget(), approvals: [approval("pending", null)] });

      const readerRead = await secureApproval(harness, "reader", "GET");
      expect(readerRead.statusCode, readerRead.body).toBe(200);

      const writerOperation = "op_approval_route_paired_writer";
      const writerResponse = await secureApproval(harness, "writer", "POST", {
        ...responseRequest,
      ...
      const readerOperation = "op_approval_route_paired_reader";
      const readerResponse = await secureApproval(harness, "reader", "POST", {
        ...responseRequest,
        operation_id: readerOperation
      });
      expectStableError(readerResponse, 403, "read_only");
      expect(harness.respondCalls()).toHaveLength(1);
      expect(harness.auditRepository.get(readerOperation)).toBeNull();

      const missingCsrfOperation = "op_approval_route_paired_missing_csrf";
      const missingCsrf = await secureApproval(
        harness,
        "writer",
        "POST",
        { ...responseRequest, operation_id: missingCsrfOperation },
        false
      );
      expect(missingCsrf.statusCode, missingCsrf.body).toBe(403);
      expect(harness.respondCalls()).toHaveLength(1);
      expect(harness.auditRepository.get(missingCsrfOperation)).toBeNull();
    } finally {
      await harness.close();
    }
  });
  ...
  it("rejects malformed wire input and adjacent methods before state, audit, or service access", async () => {
    ...
    try {
      ...
      expectStableError(
        await harness.app.inject({ method: "GET", url: `/api/v1/sessions/${sessionId}/approvals?x=1` }),
        400,
        "validation_error"
      );
      expectStableError(
      ...
      expect((await harness.app.inject({ method: "HEAD", url: `/api/v1/sessions/${sessionId}/approvals` })).statusCode).toBe(405);
      expect((await harness.app.inject({ method: "PUT", url: approvalRespondPath() })).statusCode).toBe(405);
...
async function createPairedHarness(): Promise<PairedHarness> {
  const directory = mkdtempSync(join(tmpdir(), "hostdeck-approval-route-paired-"));
  temporaryDirectories.push(directory);
  const open = openMigratedDatabase(join(directory, "hostdeck.sqlite"), { now: () => new Date(timestamp) });
  let clock = new Date(timestamp).getTime();
  const nextDate = () => new Date(clock++);
  ...
  auth.create({
    ...
    permission: "write",
    clientLabel: "Approval route writer",
  ...
  auth.create({
    ...
    permission: "read",
    clientLabel: "Approval route reader",
    createdAt: new Date(timestamp)
  });
  ...
  const audit = createHostDeckSelectedWriteAuditExecutor({
    repository: auditRepository,
    now: () => nextDate().toISOString(),
    create_record_id: () => `audit_approval_route_paired_${++auditId}`
  });
  const respondCalls: Record<string, unknown>[] = [];
  const registration = createHostDeckApprovalRouteRegistration({
    ...
    approvals: {
      ...
      async respond(intent: unknown) {
        respondCalls.push({ ...(intent as Record<string, unknown>) });
        return approval("responding", null);
      },
      async waitForTerminal() {
        return approval("approved", "approve");
      }
    } as CreateHostDeckApprovalRouteRegistrationInput["approvals"],
    audit,
    csrf,
    lock,
...
function settings(locked: boolean) {
  return {
    ...
    schema_version: 1,
    state_dir: "/tmp/hostdeck-approval-route-state",
...
async function listApprovals(harness: Pick<Harness, "app">) {
  return await harness.app.inject({ method: "GET", url: `/api/v1/sessions/${sessionId}/approvals` });
}
...
async function respondApproval(harness: Pick<Harness, "app">, payload: Readonly<Record<string, unknown>>) {
  return await harness.app.inject({ method: "POST", url: approvalRespondPath(), payload });
}

async function secureApproval(
  ...
  return await httpsExchange(
    ...
    {
      method,
      path: method === "GET" ? `/api/v1/sessions/${sessionId}/approvals` : approvalRespondPath(),
      headers: {
...
function httpsExchange(harness: PairedHarness, options: RequestOptions, body: string): Promise<HttpResult> {
  ...
  if (address === null || typeof address === "string") throw new Error("Approval HTTPS listener is unavailable.");
