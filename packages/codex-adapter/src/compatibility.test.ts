...
describe("Codex generated binding compatibility", () => {
  ...
  it("blocks unsupported and malformed Codex versions", () => {
    ...
    ).toMatchObject({ state: "incompatible", mutation_policy: "blocked", reason: expect.stringContaining("requires exactly 0.148.0") });
    expect(
      assessCodexCompatibility({
  ...
  it("blocks unreviewed schema identity before considering runtime readiness", () => {
    const binding = cloneBinding({ binding_id: "codex-app-server-0.148.0-experimental:sha256:unreviewed" });
    const result = assessCodexCompatibility({
      observed_version: "0.148.0",
      checked_at: checkedAt,
      handshake: initializedProbe(),
...
describe("Codex CLI version output", () => {
  it("parses the exact supported CLI output shape", () => {
    expect(parseCodexCliVersionOutput("codex-cli 0.148.0\n")).toBe("0.148.0");
  });

  it.each(["0.148.0\n", "codex-cli nightly\n", "codex-cli 0.148.0 extra\n", ""])('rejects malformed output "%s"', (output) => {
...
function initializedProbe(
  ...
  return {
    state: "initialized",
    user_agent: "hostdeck/0.148.0 (Ubuntu 24.4.0; x86_64)",
