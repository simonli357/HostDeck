...
const { dirname, join, relative, sep } = posix;
const linuxPathDialect = Object.freeze({
  family: "posix",
  separator: posix.sep,
  ...
  root: (path: string) => posix.parse(path).root
...
export function resolveHostDeckLocalPaths(input: PrepareHostDeckLocalPathsInput): ResolvedHostDeckLocalPaths {
  requireLinuxUid();
  const roots = resolveHostDeckPathRoots(input, linuxPathDialect);
  const leasePath = join(roots.state_dir, "hostdeck.lock");
  const appServerSocketPath = join(roots.runtime_dir, "app-server.sock");
  for (const reservedPath of [leasePath, appServerSocketPath]) {
    if (reservedPath === roots.database_path) {
      throw pathError(
        "invalid_path",
...
export function prepareHostDeckStatePaths(
  ...
  const uid = requireLinuxUid();
  const roots = resolveHostDeckStatePathRoots(input, linuxPathDialect);
  const repairs: HostDeckPathModeRepair[] = [];
  ensureSecureDirectory(roots.state_dir, "state_dir", uid, repairs);
  ensureSecureDirectory(
    dirname(roots.database_path),
    "database parent",
    uid,
    repairs
  );
  const databaseRepair = secureHostDeckRegularFile(roots.database_path, {
    label: "database",
    mode: sensitiveFileMode,
    create: true,
    repair_mode: true
...
export function inspectExistingHostDeckStatePaths(
  ...
  let directory = roots.state_dir;
  inspectExistingSecureDirectory(directory, "state_dir", uid);
  const databaseParent = dirname(roots.database_path);
  const descendants = relative(roots.state_dir, databaseParent);
...
export const linuxHostDeckLocalPathAdapter: HostDeckLocalPathAdapter =
  defineHostDeckLocalPathAdapter({
    target: "linux-x64",
    path_family: "posix",
    path_security: "uid_mode",
    resolveLocalPaths: resolveHostDeckLocalPaths,
    prepareLocalPaths: prepareHostDeckLocalPaths,
    prepareDaemonLeasePath: prepareHostDeckDaemonLeasePath,
    prepareLocalPathsAfterLease: prepareHostDeckLocalPathsAfterLease,
    prepareServiceLocalPathsAfterLease:
      prepareHostDeckServiceLocalPathsAfterLease,
    prepareStatePaths: prepareHostDeckStatePaths,
    inspectExistingStatePaths: inspectExistingHostDeckStatePaths,
    secureRegularFile: secureHostDeckRegularFile,
    openSecureRegularFile: openSecureHostDeckRegularFile,
    secureSocket: secureHostDeckSocket
  });
