...
function validateRootScripts(rootPackage, failures) {
  ...
  if (scripts["test:web"] !== expectedWebTest) {
    failures.push(
      "package.json test:web must run only selected shell, startup, approval, browser-runtime, connection-state, CSRF, host/access/lock, HTTP, Mission Control, goal control, model control, paired-device management, pairing, Plan control, prompt composer, Session Detail, SSE, and fixture tests"
    );
  }
}
