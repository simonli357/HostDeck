...
import { createScanner, formatSyntaxKind, LanguageVariant, SyntaxKind } from "typescript/unstable/ast";
...
const protocolMethodsModulePath = join(packageRoot, "src", "protocol-methods.generated.ts");
...
try {
  ...
  const protocolMethods = await readProtocolMethods(temporaryGenerated);
  ...
  if (mode === "--write") {
    ...
    await writeFile(protocolMethodsModulePath, protocolMethodsModuleSource(protocolMethods), "utf8");
    ...
    const committedProtocolMethodsModule = await readFile(protocolMethodsModulePath, "utf8");
    if (committedProtocolMethodsModule !== protocolMethodsModuleSource(protocolMethods)) {
      throw new Error("Generated Codex protocol method catalog does not match the reviewed binding unions.");
    }
...
async function readProtocolMethods(root) {
  return {
    clientRequests: await methodsIn(join(root, "ClientRequest.ts")),
    clientNotifications: await methodsIn(join(root, "ClientNotification.ts")),
    serverNotifications: await methodsIn(join(root, "ServerNotification.ts")),
    serverRequests: await methodsIn(join(root, "ServerRequest.ts"))
  };
}

async function methodsIn(path) {
  const source = await readFile(path, "utf8");
  const typeName = basename(path, extname(path));
  ...
  while (token !== SyntaxKind.EndOfFile) {
    if (token === SyntaxKind.TypeKeyword) {
      token = scanner.scan();
      if (token === SyntaxKind.Identifier && scanner.getTokenValue() === typeName) {
        token = scanner.scan();
        if (token !== SyntaxKind.EqualsToken) {
          throw new Error(`Generated protocol union ${path} type alias ${typeName} has no direct assignment.`);
        }
        foundAlias = true;
        token = scanner.scan();
        break;
  ...
  if (!foundAlias) throw new Error(`Generated protocol union ${path} is missing type alias ${typeName}.`);
  ...
  while (token !== SyntaxKind.EndOfFile && token !== SyntaxKind.SemicolonToken) {
    if (token === SyntaxKind.OpenBraceToken) {
      if (braceDepth === 0) currentMethod = null;
      braceDepth += 1;
    } else if (token === SyntaxKind.CloseBraceToken) {
      braceDepth -= 1;
      if (braceDepth < 0) throw new Error(`Generated protocol union ${path} has unbalanced object syntax.`);
      if (braceDepth === 0) {
        if (currentMethod === null) {
          throw new Error(`Generated protocol union ${path} member ${methods.length} has no method discriminator.`);
        }
        methods.push(currentMethod);
      }
    } else if (
      braceDepth === 1 &&
      (token === SyntaxKind.StringLiteral || token === SyntaxKind.Identifier) &&
      scanner.getTokenValue() === "method"
    ) {
      const colon = scanner.scan();
      const literal = scanner.scan();
      if (colon !== SyntaxKind.ColonToken || literal !== SyntaxKind.StringLiteral || !scanner.getTokenValue()) {
        throw new Error(`Generated protocol union ${path} member ${methods.length} method is not a string literal.`);
      }
      if (currentMethod !== null) {
        throw new Error(`Generated protocol union ${path} member ${methods.length} has duplicate method discriminators.`);
      }
      currentMethod = scanner.getTokenValue();
    } else if (braceDepth === 0 && token !== SyntaxKind.BarToken) {
      throw new Error(`Generated protocol union ${path} contains unexpected ${formatSyntaxKind(token)} outside a member.`);
    }
    token = scanner.scan();
  }
  if (braceDepth !== 0) throw new Error(`Generated protocol union ${path} has an unterminated member.`);
  if (methods.length === 0) throw new Error(`Generated protocol union ${path} contains no members.`);
  const unique = [...new Set(methods)].sort();
  if (unique.length !== methods.length) throw new Error(`Generated protocol union ${path} contains duplicate methods.`);
  return unique;
}

function protocolMethodsModuleSource(catalog) {
  return `// Generated by scripts/codex-bindings.mjs. Do not edit by hand.\nexport const generatedClientRequestMethods = Object.freeze(${JSON.stringify(catalog.clientRequests, null, 2)} as const);\nexport const generatedClientNotificationMethods = Object.freeze(${JSON.stringify(catalog.clientNotifications, null, 2)} as const);\nexport const generatedServerNotificationMethods = Object.freeze(${JSON.stringify(catalog.serverNotifications, null, 2)} as const);\nexport const generatedServerRequestMethods = Object.freeze(${JSON.stringify(catalog.serverRequests, null, 2)} as const);\n`;
}
